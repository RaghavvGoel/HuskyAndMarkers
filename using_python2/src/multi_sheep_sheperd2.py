...
def sheperdMovement( Sh,X,Y,ra,Pd,Pc,destination,f,mindisShep,rhoash,alpha,ang,mode,nin,ds,rs,method,predmasterslave,decrad,q,I):
    ...
    if(numpy.shape(Sh)[0] == 1):
        ...
        if(len(idx) <= len(X) and mode == 1):
            ...
            IN1 = len(IN1[0])
            if((sum(D1) < 10/10.0 or IN1 == len(X1)) and method == 1): #Why 10 ??????
...
def inertialEffect( initpoints,direction,h ):
    ...
    #print("DDDDDDDDDDDDDD ",direction)
    if numpy.size(direction)!=0:
...
def clusteringEffect(initpoints,n,c,rs,Shepherd): #the clustering of the sheep
    ...
    for j in range(Shepherdsize[0]):
        ...
        for i in range(numpy.size(closeagents)):
            ...
            #orderedD = numpy.sort(D) #extra 
            #D_less_than_10 = numpy.where((orderedD<5.0)) #extra 
            #n = len(D_less_than_10) #extra 
            #print("NNNNNNNNNNNNNNNNNNNNNNNNNN")
            #print("no. of less than 5: " , n)
            #sortedDindex=numpy.where((sortedD<=n))
...
def sheepmovements(initpoints,n,c,rs,Shepherd,ra,rhoa,rhos,unitvec,h,e,dps,mode,destination,p,dist_threshold,sheep_human_agents,strength_human,reached):
    global done
    ...
    #ATTRACTION TOWARDS Human Dest +++++++++++++++++++++++++++++++++++++++++++++++++++++++
    if(rviz_pos[0] != 10000 and reached != 1):
        #for i in range(len(initpoints[0])):
        #     if(sqrt(pow(initpoints1[0][i] - rviz_pos[0],2) + pow(initpoints1[1][i] - rviz_pos[1],2))):
        # if(done != 1):
        #     dist_human = numpy.linalg.norm(initpoints.T-rviz_pos,axis=1)
        #     sorted_ind = numpy.argsort(dist_human)
        #     print("_____----------_______________-----------------____________--------")
        #     print(sorted_ind)
        #     print(floor(len(initpoints[0])/2) , " size of sorted_ind: " , numpy.size(sorted_ind))                        
        #     sheep_human_agents = sorted_ind[floor(len(initpoints[0])/2)]
        #     strength_human = 3
        #     done = 1
        #Strrngth of this will more than others
        #So let's keep it: 3
        rviz_pos_temp = numpy.asarray(rviz_pos).reshape(1,2) 
        for i in sheep_human_agents:
            print("i: " ,i , " initpoints[:,i]: " , initpoints[:,i] , rviz_pos_temp)
            print("size_ini: " , numpy.size(initpoints[:,i]) , " rviz_size: " , numpy.size(rviz_pos_temp))
            print("shape_ini: " , numpy.shape(initpoints[:,i]) , " rviz_shape: " , numpy.shape(rviz_pos_temp))
            initpoints[:,i],A2 = move(initpoints[:,i],rviz_pos_temp[0],strength_human,1)
...
def main():
    ...
    scalar = 1;
    global rviz_pos, done;
    e=0.3/scalar # strength of error movement
    p=0.05/scalar # probability of an error
    Xstartpos=0 #swarm initial position
    Ystartpos=0

    

    h = 0.5/scalar #strength of inertial force
    N = 50 # number of agents
    N1 = copy.deepcopy(N)
    ra= 2.0/scalar  # distance for self repulsion
    mindisShep = 6
    n=numpy.ceil(0.9*N1)
    Pd = ra*log(N)
    Pc = ra
    threshold = 10.0
    dps= 1.0 # speed of agents
    
    f = (N^(1/2))/scalar
    rhoa=2.0/scalar # strength of self repulsion 
    rhos=1.0/scalar #strength of predatory repulsion 
    q = 1.8
    #I = []
    rhoash = 1.0/scalar
    #Shepherd=[-0,100]
    ...
    c=1.05/scalar #strength of clustering
    ...
    shape = Marker.CUBE;
    for i1 in range(N+nos+1): #to include shepherd too
        marker = Marker()
        print("i1 " , i1)
    ...
    while not rospy.is_shutdown():
        ...
        if(rviz_pos[0] != 10000 and done != 1):
            dist_human = numpy.linalg.norm(initpoints.T-rviz_pos,axis=1)
            sorted_ind = numpy.argsort(dist_human)
            sheep_human_agents = sorted_ind[:floor(N/2)]
            done = 1;   
            print("________________---------------------_________________---------------_____")
            print(sorted_ind)
            print(floor(N/2) , " size of sorted_ind: " , numpy.size(sorted_ind))
            print(sheep_human_agents)
            for i in sheep_human_agents:
                M.markers[i].color.r = 1.0
            ...
            #if(i < N):
            marker.pose.position.x = rviz_pos[0];            

            marker.pose.position.y = rviz_pos[1];
            ...
            marker.scale.x = 2*threshold1;
            marker.scale.y = 2*threshold1;
            marker.scale.z = 0.1;
        ...
        if(rviz_pos[0] != 10000):
            if(len(sorted_ind) != 0):
                dist_human_chosen = numpy.linalg.norm(initpoints[:,sheep_human_agents].T-rviz_pos,axis=1)
                agents_reached = numpy.where(dist_human_chosen < threshold1)
                if(numpy.size(agents_reached) >= floor(0.9*len(sheep_human_agents))):
                    reached = 1;  
                    rviz_pos = [10000 , 10000]
                    done = 0
                    reached = 0      
        ...
        sheepmovements(initpoints,n,c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode,destination,p,dist_threshold,sheep_human_agents,strength_human,reached)
        ...
        # th1 = numpy.arctan2(Sh[0][1]-tempSh[0][1],Sh[0][0]-tempSh[0][0])
        # th1 = th1*(180)/(22.0/7)
        # if(th1 < 0):
        #     th1 = th1%360;
        
        # th2 = numpy.arctan2(Sh[1][1]-tempSh[1][1],Sh[1][0]-tempSh[1][0])
        # th2 = th2*(180)/(22.0/7)
        # if(th2 < 0):
        #     th2 = th2%360;
        # print("77777777777777777777777777777777")    
        # print("ang1: " , th1 , " ang2: " , th2 )
        # print("dist between Sh: " , sqrt(pow(Sh[1][1] - Sh[1][0],2) + pow(Sh[0][1] - Sh[0][0],2)))
        #print("I : " , I)
        ...
        for i in range(nos):    
            ...
            #for i in sheep_human_agents:
                #M.markers[i].color.r = 1
            
            #ADDING A CIRCLE AT THE CLICKED POINT TOO :p
        # marker = Marker()
        # print("i " , i)
        # marker.header.frame_id = "/multi_sheep_sheperd";
        # marker.header.stamp = rospy.get_rostime();
