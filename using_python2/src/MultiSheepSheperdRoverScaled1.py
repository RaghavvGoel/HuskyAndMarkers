...
def main():
    ...
    while(heading_angle1 < 85 or heading_angle1 > 95):
        print("in while " , heading_angle1)            
    ...
    while not rospy.is_shutdown():
        ...
        dx1 = (Sh[0,0] - temp_sh[0,0])
        dy1 = (Sh[0,1] - temp_sh[0,1])
        V1 =  sqrt(pow(dx1,2) + pow(dy1,2))
        theta1 = numpy.arctan2(dy1,dx1)        
        #rov.position.x = ini_lat + dx
        #rov.position.y = ini_lon + dy
        print("V1: " , V1 , " " , theta1)
        rov1.twist.linear.x = V1        
        ...
        while(heading_angle1 < beta1 - 10 or heading_angle1 > beta1 + 10):
            print("beta1: " , beta1 , "heading_angle1 " , heading_angle1)            
            ...
            rov1.twist.angular.z = yaw_to_publish         
            rvel1.publish(rov1)
            r1flag.publish(flag1_inwhile)
        ...
        while(rospy.get_rostime() < time1 + rospy.Duration(1.5)):
            print("in time loop")
            rov1.twist.linear.x = 12.0
            rov1.twist.angular.z = 00.0
            rvel1.publish(rov1)
            r1flag.publish(flag1_inwhile)
