...
def initPhysico(initpoints,V1, deltaT,S,leaderidx,rs,dps,rhos,ra,destination,destinationthreshold,destination1,destination2,swarm_split):
...
def main():
    ...
    #destination = [0,50]
    destination1 = [-45.0,20.0]
    destination2 = [45.0,20.0]    
    ...
    while not rospy.is_shutdown():
        ...
        initpoints,V1 = initPhysico(initpoints,V1, deltaT,Sh,leaderidx,rs,dps,rhos,ra,destination,destinationthreshold/3.0,destination1,destination2,
            swarm_split)
        print("yolo")
        #print("x: " , x)
        #print("y: " , y)
        ...
        if(swarm_split == 1 and eaten1 == 0):
            M.markers[N+nos].scale.x = M.markers[N+nos].scale.x - sp*(numpy.size(near_dest1))         
            M.markers[N+nos].scale.y = M.markers[N+nos].scale.y - sp*(numpy.size(near_dest1))
            if(M.markers[N+nos].scale.y < 0.2):
                eaten1 = 1;

        if(swarm_split == 1 and eaten2 == 0):
            M.markers[N+nos+1].scale.x = M.markers[N+nos+1].scale.x - sp*(numpy.size(near_dest2))         
            M.markers[N+nos+1].scale.y = M.markers[N+nos+1].scale.y - sp*(numpy.size(near_dest2))
        ...
        if(numpy.size(near_dest) >= 0.9*(N)):
            ...
            print("stop time:" ,stop_time.to_sec() )
            print("start_time: " , start_time.to_sec() )
            rospy.loginfo("STRT time %i %i", start_time.secs, start_time.nsecs)
            rospy.loginfo("STOP time %i %i", stop_time.secs, stop_time.nsecs)
