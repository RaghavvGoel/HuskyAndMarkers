...
def main():
    rospy.init_node('multi_sheep_sheperd3', anonymous=True)
    #pub = rospy.Publisher('chatter', String, queue_size=10)
    #sub = rospy.Subscriber("chatter",String,callback)
    pub_marker = rospy.Publisher('multi_sheep_sheperd3', MarkerArray , queue_size=1)
    ...
    for mc1 in range(MC):
        ...
        while not rospy.is_shutdown():
            ...
            if(toggle == 0):
                ...
                if(numpy.size(g1)   and eating1 == 0):
                    print("GRP1")
                    ...
                    if(orient1 == 1 and leader_front1 == 0):
                        ...
                        if(numpy.size(sub_n1) > 0):
                            ...
                            S0,a = moveSheperd(S0,GCM1,1.25,-1)    
                ...
                if(len(g2) > 0 and eating2 == 0):
                    ...
                    if(orient2 == 1 and leader_front2 == 0):
                        ...
                        if(numpy.size(sub_n2) > 0):
                            #print("avg: " , numpy.average(initpoints[:,g1]))
                            S1,a = moveSheperd(S1,GCM2,1.25,-1)                                    
                ...
                if(out1 == 1 and out2 == 1):
                    ...
                    for i in range(N):
                        ...
                        if(sqrt(pow(initpoints[0][i] - destination[0],2) + pow(initpoints[1][i] - destination[1],2)) > threshold+5):                    
                            outthreshold.append(i)

                    #g1 = numpy.intersect1d(grp1,outthreshold) present above
                    ...
                    if((in1 == 1 and in2 == 1) or (out1 == 1 and out2 == 1)):
            ...
            if(out1 == 0):
                #print()
                near_dest1 = N - numpy.size(outthreshold1)
                print("NEAR DEST1 : " , near_dest1)
                print(size_dec/N*numpy.size(near_dest1))
                M.markers[N1+nos].scale.x = M.markers[N1+nos].scale.x - size_dec/N*(near_dest1)
                M.markers[N1+nos].scale.y = M.markers[N1+nos].scale.y - size_dec/N*(near_dest1)
            ...
            if(out2 == 0):
                near_dest2 = N - numpy.size(outthreshold2);
                M.markers[N1+nos+1].scale.x = M.markers[N1+nos+1].scale.x - size_dec/N*(near_dest2)
                M.markers[N1+nos+1].scale.y = M.markers[N1+nos+1].scale.y - size_dec/N*(near_dest2)
            ...
            #if(eaten1 == 1 and eaten2 == 1): #START Feeding
                #in_g1 = numpy.intersect1d(near_dest1,grp1)
            if(numpy.size(temp_grp1) > 0):
                dT3 = numpy.linalg.norm(initpoints.T - destination,axis = 1);
                near_dest = numpy.where(dT3 < threshold + 5)
                in_g1 = numpy.intersect1d(temp_grp1,near_dest[0])
                in_g2 = numpy.intersect1d(temp_grp2,near_dest[0])
                if(numpy.size(in_g1) > 0 and numpy.size(in_g2) > 0):
                    M.markers[N1+nos+2].scale.x = M.markers[N1+nos+2].scale.x - size_dec/N*numpy.size(near_dest)
                    M.markers[N1+nos+2].scale.y = M.markers[N1+nos+2].scale.y - size_dec/N*numpy.size(near_dest)
            ...
            if(count > 3500):
                ...
                if(eaten3 != 1 and out1 == 1 and out2 == 1):
                    ...
                    agent_sep[0][mc1] = N - numpy.size(near_dest) - near_dest1 - near_dest2
                    task[0][mc1] = 4                
                agent_sep[0][mc1] = N - numpy.size(near_dest) - near_dest1 - near_dest2    
                break    
            rate.sleep()
