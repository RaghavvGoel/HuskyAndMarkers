...
def SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,leaderidx,leaderRrep,leaderRori,leaderRatt,destination,destinationthreshold):
    ...
    for i in range(len(initpoints[0])):
        ...
        if(flag != -1):
            ...
            # if(flag == -1):
            print("leaderidx: " , leaderidx , " " , leaderidx)
            ...
            #agents will be only repelled from other agents and not from leaders
            else:
                v = numpy.where((dists <= Rrep))
                #WHY are we reppelin agents from leaders ???   
            ...
            if(i != N):  
                ...
                if(len(dist_leader) != 0):
                    ...
                    v = v13[idx1]
                    const = 3 #more attraction towards leader
                    #for j in range(numpy.size(v)):
            ...
            if(dpredFlag == 1):
                ...
                if(numpy.intersect1d(i,leaderidx) == i):
                    di = di + (dleader - dr)
                    nagentFlag = 1
                #elif(drFlag == 1 and dleaderFlag == 1):
                    #di = di - dr + dleader
                    #nagentFlag = 1
                else:

                    print("hoho: "  , dr[0] , " " , zoo[0] , " " , da[0])
                    #di[0] = di[0] + (-(0.25)*dr[0] + ((0.25)*zoo[0] + (0.5)*da[0]))
                    #di[1] = di[1] + (-(0.25)*dr[1] + ((0.25)*zoo[1] + (0.5)*da[1])) 
                    di[0] = di[0] + (-dr[0] + (zoo[0] + da[0]))
                    di[0] = di[0] + (-dr[0] + (zoo[0] + da[0]))
                    nagentFlag = 1;                      
                # elif(drFlag == 1):
                #     di = di - dr;
...
def main():
    ...
    nol = 0
    #randomly calling the leader near the swarm
    l1 = numpy.random.rand(1,N)
    l1 = [0 , 5]
    L = numpy.asarray(l1).reshape(1,2)
    leaderidx = N-1
    initpoints[0][N-1] = l1[0]
    initpoints[1][N-1] = l1[1]
    # nl = 0
    ...
    for i in range(N+nol+1): #to include shepherd too
        ...
        if(i < N):
            ...
            marker.pose.position.y = initpoints[0][i];
        ...
        if(i == N-1):
            marker.ns = "shep"
            #DO change this afterwords, leaderidx keeps only the indices
            marker.pose.position.x = initpoints[0][i];
