...
def SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,leaderidx,leaderRrep,leaderRori,leaderRatt,destination,destinationthreshold,nol):
    pos= initpoints.T
    #print("pos:")
    #print(pos)
    flag = -1;
    ...
    for i in range(len(initpoints[0])):
        ...
        for k in idx[0]:
            if(i == k):
                ...
                flag = -1    
        print("Flag forou of destinationthreshold: " , flag)
        if(flag != -1):
            ...
            if(nagentFlag > 0):
                ...
                dtheta = numpy.arccos(((di[0][1]*vels[i][1]) + (di[0][0]*vels[i][0])))
...
def main():
    ...
    tempdi = numpy.zeros((NT,2))
    temppos = numpy.zeros((NT,2))
    mu = -5;
    sigma = 2;
    x = Xstartpos + numpy.random.normal(mu,sigma,(1,NT))
    y = Ystartpos + numpy.random.normal(mu,sigma,(1,NT))
    ...
    leaderRrep = Rrep    
    leaderRori = Rrep + 30 #The agents between the distance of 10(leaderRori) and 20(leaderRatt) get attracted towards them
    leaderRatt = 0 # We
    ...
    shape = Marker.CUBE;
    for i in range(NT+1): #to include shepherd too
        marker = Marker()
        print("i " , i)
        ...
        if(i >= N and i<N+nol):
            marker.ns = "shep"
            marker.pose.position.x = initpoints[0][i];
            marker.pose.position.y = initpoints[1][i];
            marker.color.r = 1.0;
            marker.color.g = 0.0;
    ...
    while not rospy.is_shutdown():
        ...
        #print(vels)
        initpoints,vels =  SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,leaderidx,leaderRrep,leaderRori,leaderRatt,destination,destinationthreshold,nol)
