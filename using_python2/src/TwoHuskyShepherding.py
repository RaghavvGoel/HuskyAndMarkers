...
def singlesheperdMovement(S,X,Y,ra,Pd,Pc,destination,f):
    ...
    if(len(idx) == 0):
        ...
        dist_from_dest = []
        for i in range(l[0]):
            dist_from_dest.append(numpy.sqrt(numpy.square(X[i] - destination[0]) + numpy.square(Y[i] - destination[1])))
        sorted_arg_dist_from_dest = numpy.argsort(dist_from_dest)
        for i in range(l[0]):  # or len if X is a list and not an array     
            Dgcm.append(numpy.sqrt(numpy.square(X[i] - Xgcm) + numpy.square(Y[i] - Ygcm)))
            #dist_from_dest.append(numpy.sqrt(numpy.square(X[i] - destination[0]) + numpy.square(Y[i] - destination[1])))
            #print(i, " dgcm should have some: " , Dgcm[i])
            if(Dgcm[i] > f and i != sorted_arg_dist_from_dest[0]): # and i != sorted_arg_dist_from_dest[0]):
                ind.append(i)
        
        #now if the one having max distance is closest to dest then skip that one
        #if(Dgcm.index(max(Dgcm)) == sorted_arg_dist_from_dest[0]):

        if(len(ind) == 0):
            print("entering singleherdingPosition:")
            S = singleherdingPosition(S,Pc,X,Y,destination)
            check_mode = 1;
            #print("After HERDING: aka new value " , S , " hhhhhhhhhhhhhhhhhhhhhh" )
            #currentmode = 1;            
        else:
            check_mode = 0; #for herding or collectng
            pos = Dgcm.index(max(Dgcm))
            last = [X[pos],Y[pos]]
            print("entering singlecollectingPosition:")
            S = singlecollectingPosition(S,last,Xgcm,Ygcm,Pd)
            #print("AFTER singlecollectingPosition: " , S , " ccccccccccccccccc")
            #currentmode = 2             
    return S
            
def singleherdingPosition(S,Pd,X,Y,destination):
    #print("In HERDING " , S)
    #S2 = numpy.copy(S)
    #S2 = [0,0]
    l = numpy.shape(X)
    ...
    if(len(idx) != 0):
        agent = numpy.zeros(2)
        agent[0] = numpy.mean(X) #X[pos]
        agent[1] = numpy.mean(Y) #Y[pos]
        ...
        S,s2 = moveSheperd(agent,destination[:],Pd,-1) #s2 is useless
        #S,s2 = move(S,S,0,0)
        #S = S1
...
def singlecollectingPosition( S,last,XGCM,YGCM,Pc):
    ...
    l,s = moveSheperd(numpy.asarray(last),G,Pc,-1)
...
def clusteringEffect(initpoints,n,c,rs,Shepherd,swarm_split): #the clustering of the sheep
    ...
    for j in range(Shepherdsize[0]):
        ...
        #print(initpoints)
        #print(initpoints.T)    
        #we should be having clusterpoints here instead of initpoints
        #D=numpy.linalg.norm(initpoints.T-S,axis=1) 
        D=numpy.linalg.norm(clusterpoints.T-S,axis=1)
        ...
        for i in range(numpy.size(closeagents)):
            ...
            agent1=clusterpoints[:,closeagents[0][i]] 
            D=numpy.linalg.norm(clusterpoints.T-agent1,axis=1)
            ...
            agent2=(clusterpoints[:,sortedDindex[0]]).mean(1)
            # print("())))))))))))))))))))))))))))))))))))))))))))")
            # print(closeagents)
            # print(orderedD)
...
def selfRepellingEffect( initialpoints,ra,rhoa):
    ...
    for i in range(l[1]):
        D =numpy.linalg.norm(repelledpoints.T- repelledpoints[:,i],axis=1)
        closeagents=numpy.where((D>0) & (D<ra)) 
        if numpy.size(closeagents)!=0:
            agent1=numpy.reshape(numpy.repeat(repelledpoints[:,i],numpy.size(closeagents)),(2,numpy.size(closeagents)))
            agent1,agent2=move(agent1,repelledpoints[:,closeagents[0]],rhoa,-1)
...
def main():
    ...
    #rospy.Subscriber('/Husky22/base_pose_ground_truth', Odometry, position_husky22)

#Final Destinations: 
#('dest1: ',)
    destination1x = [-35 , -71. ,-108. ,-140. , -63. , -82. , -78. ,-116. , -63. , -96., -119. , -71.,  -109. , -62. ,-105. , -91. ,-142. , -68. ,-128. ,-133. ,-136. , -80. ,-128. , -59.,
  -130.]
...
#('dest2: ',)
    destination2x = [ 35. , 198. , 169. , 218.,  179. , 171.,  223.,  187.,  240. , 212. , 232. , 239.,
...
#('dest: ',)
   ...
   -56.  , 40. , -24. , -26. ,  15. ,   7. , -71. ,  -8.,   61. ,  57. ,  21.,  -38.,
    74.]
    destinationy = [  80. , 238. , 159.,  -76. , -20. ,-209. ,  31. , -59. ,-171.  , -6. ,-180. , -20.,
   ...
   142.]
    ...
    for mc1 in range(MC):            
        ...
        N1 = copy.deepcopy(N)
        ra=3.00  # distance for self repulsion
        mindisShep = 6
        n=int(numpy.ceil(0.25*N1)); #taking the floor
        #n = (N*90/100)/2 #as we'll have two groups after this
        ...
        c= 1.05 #1.05 #strength of clustering
        ...
        #H = [Twist(), Twist(), Twist(), Twist(), Twist(), Twist(), Twist(), Twist()]
        #H = [Twist(), Twist(), Twist(), Twist(), Twist(), Twist(), Twist(), Twist()]
        H = []
        for i in range(N):
            H.append(Twist())
        ...
        in1 = 0; in2 = 0; 
        eating1 = 0 ; eating2 = 0; eaten3 = 0; eaten1 = 0; eaten2 = 0;
        ...
        inthreshold1 = []; inthreshold2 = []; inthreshold =[]
        dest3 = [0,0]
        too_close = 0;
        ...
        dist_left_to_cover_sheph = numpy.zeros(nos)
        ...
        while not rospy.is_shutdown():
            ...
            # pub_name = "";
            # for i in range(N):
            #     pub_name = "pub_vel_husky" + "i"
            #     pub_name.publish(H[i])
            ...
            if(toggle == 0):
                ...
                if(out1 == 0 or out2 == 0):
                    ...
                    if(len(g1) >= 0.1*(numpy.size(grp1)) and eating1 == 0):
                        ...
                        #print("g1: " , g1, " " , g1[0])
                        n11 = numpy.size(grp1)
                        f1 = (ra)*pow(len(grp1),0.5) #(len(grp1) - 1)
                        #print("f1: " , f1)
                        #if(n11 > 0):
                        Pd1 = ra*log(len(grp1))
                        #else:
                        ...
                        if not(numpy.linalg.norm(dest1 - Sh[0]) <= threshold_for_sub_destination): #(Sh[0][0] <= dest1[0] and Sh[0][1] >= dest1[1]):
                            ...
                            if(numpy.where(D10 < rs/3)):
                                S00, a = moveSheperd(S0[0],numpy.asarray(destination1).reshape(1,2),1.5,-1)
                    ...
                    if(len(g2) >= 0.1*(numpy.size(grp2)) and eating2 == 0):
                        ...
                        n22 = numpy.size(grp2)
                        f2 = (ra)*pow(len(grp2),0.5) #(len(grp2) - 1)
                        #if(len(grp2) > 0):
                        Pd2 = ra*log(len(grp2))
                        # else:
                        #     Pd2 = 0;    
                        ...
                        if not (numpy.linalg.norm(dest2 - Sh[1]) <= threshold_for_sub_destination): #(Sh[1][0] >= dest2[0] and Sh[1][1] >= dest2[1]):
                            ...
                            if(numpy.where(D20 < rs/3)):
                                S10, a = moveSheperd(S10,numpy.asarray(destination2).reshape(1,2),1.5,-1)
                ...
                if(out1 == 1 and out2 == 1 and (in1 == 0 or in2 == 0)): #and pos_update1 == 1 and pos_update2 == 1
                    #print("|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||")
                    if(enter_once2 == 0):
                        mode1 = 2 #redelaration for herding again
                        mode2 = 2;           
                        predmasterslave1 = 3;                                     
                        predmasterslave2 = 3
                        method1 = 1;
                    ...
                    if(len(g1) > ceil(0.1*len(grp1)) and in1 == 0 and too_close == 0):
                        print("Sh0 moving sheep to task3")
                    ...
                    if(len(g2) > ceil(0.1*len(grp2)) and in2 == 0 and too_close == 0):#if too_close becomes 1, then in2 will become 1
                        print("Sh1 moving sheep to task3")
                        f2 = pow(len(grp2),0.5)
                        Pd2 = ra*log(len(grp2))
                        #print("A3")
            ...
            for i in range(N):
                #if(toggle == 1):
                    #print(i)
                #overwriting the unitvec and movement for sheep inside the threshold    
                # if(len(inthreshold1) > 0 or len(inthreshold2) > 0):    
                #     if(i in inthreshold1):
                #         agent, a = move(initpoints[:,i],destination1,1,1)
                #         print("1: " , agent)
                #         diff = agent - initpoints[:,i];
                #         norm = numpy.linalg.norm(diff, axis = 0)
                #         unitvec[0,i] = diff[0]/norm
                #         unitvec[1,i] = diff[1]/norm
                #         initpoints[:,i] = agent                        
                #     elif(i in inthreshold2):
                #         agent, a = move(initpoints[:,i],destination2,1,1)
                #         print("2    : " , agent)
                #         diff = agent - initpoints[:,i];
                #         norm = numpy.linalg.norm(diff, axis = 0)
                #         unitvec[0,i] = diff[0]/norm
                ...
                if(huskyX[i] < 0 and out1 == 0):
                    print(i,huskyX[i], " " , delta_theta[i]*180/pi)
                ...
                #print(i, " sheep:",delta_theta[i]*180/pi)
                temp_initpoints2 = copy.deepcopy(initpoints)
                ...
                # if(eating1 == 0 or eating2 == 0):
                if(i in inthreshold1 or i in inthreshold2):
                    speed_sheep[i] = 0.25*dist_left_to_cover[i];
                #delta_theta[i] = 0.0                   
                elif(numpy.size(inthreshold) > 0 and  i in inthreshold[0]):
                    speed_sheep[i] = 0.25*dist_left_to_cover[i]
                elif(eating1 == 1 and eating2 == 1):
                    speed_sheep[i] = 0.25*dist_left_to_cover[i]    
                else:
                ...
                #     speed_sheep[i] = 0.25*dist_left_to_cover[i]
                # else:
                #     speed_sheep[i] = 0.5*dist_left_to_cover[i]

                H[i].linear.x = speed_sheep[i]
                H[i].angular.z = k*(delta_theta[i])
                #print(i,speed_sheep[i], dist_left_to_cover[i])
                #print("i: " , i)
            ...
            for i in range(nos):    
                ...
                if(toggle == 1):
                    ...
                    #print("delta: " , delta_theta_shepherd[i]*180/pi)
                    if(abs(delta_theta_shepherd[i]) > angle_bound and i == 0 and out1 == 0):
                        delta_theta_shepherd[i] = numpy.sign(delta_theta_shepherd[i])*angle_bound
                        if(i == 0):
                            delta_theta_shepherd[i] = -1*delta_theta_shepherd[i]  #numpy.sign(delta_theta_shepherd[i])*((2*pi)%abs(delta_theta_shepherd[i]))    

                    if(abs(delta_theta_shepherd[i]) > 2*angle_bound):
                        delta_theta_shepherd[i] = -numpy.sign(delta_theta_shepherd[i])*(2*pi - abs(delta_theta_shepherd[i]))   


                    dist_left_to_cover_sheph[i] = numpy.linalg.norm(Sh[i] - numpy.asarray([husky_predX[i], husky_predY[i]]))
                    ...
                    sheperd_speed[i] = 0.75 #*dist_left_to_cover_sheph[i]
                    pred_dist_from_sheep = numpy.linalg.norm(initpoints.T - Sh[i] , axis = 1)
                    sheep_within_ra = numpy.where(pred_dist_from_sheep <= ra) #3*ra as per Strombom
                    sheep_within_more_than_ra = numpy.where(pred_dist_from_sheep <= (ra+2))
                    # if(numpy.size(no_of_sheep_within_5m) > 0):
                    #     sheperd_speed[i] = 0.0 #to prevent the predator hitting the sheep
                    if(numpy.size(sheep_within_more_than_ra) > 0):
                        #speed_sheep[no_of_sheep_within_1m[0][0]] = 0.6#increasing the speed of sheep to prevent sticking with the predator
                        #H[sheep_within_1m[0][0]].linear.x = 0.6 
                        #sheperd_speed[i] = 0.25
                        if(numpy.size(sheep_within_ra) > 0):
                            sheperd_speed[i] = 0.0
                        else:        
                            slow_speed_for_some_dist = True;
                        #for movingshepherd slowly after stopingg     
                        if(sheperd_speed[i] == 0.0 and slow_speed_for_some_dist):
                            sheperd_speed[i] = 0.25
                    else:
                        slow_speed_for_some_dist = False
                    #print("eating1:" , eating1)
                    #print("eating2:" , eating2)            
                    #H3.linear.x = sheperd_speed;    
                    if(eating1 == 1 and out1 == 0): #if this is one, then the shpehrd will try to move behind the swarm for the new destination
                        sheperd_speed[0] = 0.75
                    if(eating2 == 1 and out2 == 0):
                        sheperd_speed[1] = 0.75    
                M.markers[N1+i].pose.position.x = husky_predX[i]  #Sh[i][0]
                M.markers[N1+i].pose.position.y = husky_predY[i]  #Sh[i][1]
                ...
                #FOR ASSIGNING NEW SHEEP SPEED FOR SHEEP TOO CLOSE:
            ...
            if(out1 == 0):
                ...
                in_dest1 = len(inthreshold1) #N/2 - numpy.size(outthreshold1)
                # if(in_dest1 == 0 and eating1 == 1):
                #     in_dest1 = 1;
                print("inthreshold1" , inthreshold1)
                if(in_dest1 >= N/4 or eating1 == 1): 
                    eating1 = 1;
                    M.markers[N1+nos].scale.x = M.markers[N1+nos].scale.x - (0.04*in_dest1)
                    M.markers[N1+nos].scale.y = M.markers[N1+nos].scale.y - 0.04*(in_dest1)
            ...
            if(out2 == 0):
                ...
                in_dest2 = len(inthreshold2) #N/2 - numpy.size(outthreshold2)
                # if(in_dest2 == 0 and eating2 == 1):
                print("inthreshold2" , inthreshold2)
                #     in_dest2 = 1;
                if(in_dest2 >= N/4 or eating2 == 1):  
                    eating2 = 1                  
                    M.markers[N1+nos+1].scale.x = M.markers[N1+nos+1].scale.x - (0.04*(in_dest2))
                    M.markers[N1+nos+1].scale.y = M.markers[N1+nos+1].scale.y - (0.04*(in_dest2))
            ...
            n2 = numpy.size(numpy.where(D1 < rs/3)) #rs = 15 => rs/3 = 5
            n3 = numpy.size(numpy.where(D2 < rs/3)) #5 should be enough for gazebo based
            # print("??????????????????????????????????????????")
            ...
            if(abs(Sh[0][1] - Sh[1][1]) < 6 and enter_once == 0 and husky_predY[0] != 0 and husky_predY[1] != 0): #the rpedators start from the x axis
                switch = 0;
                toggle = 1;
                #crossed_swarm = 1;
                enter_once = 1;
            ...
            if(out1 == 0):            
                ...
                for i in range(N):
                    ...
                    if(sqrt(pow(initpoints[0][i] - destination1[0],2) + pow(initpoints[1][i] - destination1[1],2)) > threshold*(k1)):
                        #flag = flag + 1
            ...
            if(out2 == 0):     
                ...
                for i in range(N):
                    ...
                    if(sqrt(pow(initpoints[0][i] - destination2[0],2) + pow(initpoints[1][i] - destination2[1],2)) > threshold*(k1)):
                        #flag = flag + 1
            ...
            outthreshold = numpy.where(dist_from_dest > threshold )
            inthreshold = numpy.where(dist_from_dest <= threshold ) #+5
            # for i in range(numpy.size(inthreshold)):
            #     inthreshold_list.append(in)
