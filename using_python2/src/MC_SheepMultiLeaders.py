...
def clusteringEffect(initpoints,n,c,rs,Shepherd,leader_on): #the clustering of the sheep
    ...
    for j in range(Shepherdsize[0]):
        ...
        # else:
        #     closeagents=numpy.where((D>=rs))  # increased attrction radius from 12.5 to 15    
        ...
        if(leader_on == 0):    
            ...
            closeagents=numpy.where((D>rs)) 
            closeagents_nearleaders = numpy.where(D <= rs)
            if(numpy.size(closeagents_nearleaders) > 0):
                for i in range(numpy.size(closeagents)):
                    #print("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF")
                    agent1=initpoints[:,closeagents[0][i]] 
                    D=numpy.linalg.norm(initpoints.T-agent1,axis=1)
                    ...
                    orderedD = numpy.sort(D)
                    #indices of all the nieghbours within 5m of an agent
                    D_less_than_10 = numpy.where((orderedD < 4.0))
                    #if(swarm_split != 1):
                    n = len(D_less_than_10[0])
                    sortedDindex=sortedD[:n]
                    sortedDindex= numpy.asarray(sortedDindex).reshape(1,n)
...
def sheperdingEffect(initpoints,Shepherd,rs,rhos,leader_on): #the fear of the predator
    Shepherdsize=numpy.shape(Shepherd)
    ...
    for j in range(Shepherdsize[0]):
        ...
        NS,shepherdingpoints[:,closeagents[0]]=move(S,shepherdingpoints[:,closeagents[0]],rhos,-1+2*leader_on)
...
def sheepmovements(initpoints,n,c,rs,Shepherd,ra,rhoa,rhos,unitvec,h,e,dps,mode,destination,p,leader_on):
    print("IN SHEEPMOVEMENT >>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    print("sheperd: " , Shepherd)
    copypoints = copy.deepcopy(initpoints)
    if(n != 1):
        #if(swarm_split == 1):
        initpoints = clusteringEffect(initpoints,n,c,rs,Shepherd,leader_on)
    idx1 = []
    X = initpoints[0,:]
    Y = initpoints[1,:]
    for i in range(len(X)):
        ...
        if(dist >= 10):
            idx1.append(i)
    initpoints1 = sheperdingEffect(initpoints,Shepherd,rs,rhos,leader_on)
    for i in range(len(initpoints[0])):
        if(i not in idx1):
...
def leadermovement(leader_pos,initpoints,destination,rs,rrep,step):
    ...
    idx_att = numpy.where(D <= 10.0)#NO USE
    ...
    if(numpy.size(idx_rep) > 0): #let's change from 0 to sth  else ????
        ...
        #leader_pos , a = moveSheperd(leader_pos,initpoints[:,idx_rep[0][0]],1.5,-1) #idx[0] implies the nearest agent to the leader
        leader_pos , a = moveSheperd(leader_pos,destination,1.25,1)
    else:
        leader_pos , a = moveSheperd(leader_pos,destination,1,1)
    #leader_pos, a = stepspertimestepLeader(leader_pos,temp_leader_pos,1.5)     
    return leader_pos  

def main():
    rospy.init_node('multi_sheep_sheperd2', anonymous=True)
    #pub = rospy.Publisher('chatter', String, queue_size=10)
    #sub = rospy.Subscriber("chatter",String,callback)
    pub_marker = rospy.Publisher('multi_sheep_sheperd2', MarkerArray , queue_size=1)
   ...
   142.]
    ...
    for mc1 in range(MC):
        ...
        for i1 in range(N+nos+nod): #to include shepherd too
            marker = Marker()
            print("i1 " , i1)
            marker.header.frame_id = "/multi_sheep_sheperd2";
        ...
        in1 = 0; in2 = 0; 
        eating1 = 0 ; eating2 = 0; eating3 = 0 ; eaten3 = 0;
        ...
        while not rospy.is_shutdown():
            ...
            swarm_split = 0
            print("toggle: " , toggle , " swarm_split: " , swarm_split)
            if(toggle == 0):
                ...
                if(len(g1) > 0.1*(numpy.size(grp1)) and eating1 == 0):
                    print("GRP1")
                    #print("orient1: " , orient1 , " leader_front1: " , leader_front1)
                    print("leader1: " , Sh[0] , " " , "sub_dest1:",sub_dest1)
                    #sheepmovements(initpoints,((len(g1))),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode1,destination1,p,swarm_split)
                    #sheepmovements(initpoints[:,grp1],0.9*len(grp1),c,rs,Sh[0].reshape(1,2),ra,rhoa,rhos,unitvec,h,e,(dps/5.0),mode,destination1,p)
                    #Replace sheperdMovement with leader movement, orient the leader in the front of the swarm
                    
                    if(orient1 == 0 and leader_front1 == 0): #+++++++++++++++++++++++++++++++++++++++++++++++++++++++
                        sheepmovements(initpoints,((len(g1))),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode1,destination1,p,swarm_split)
                        GCM1 = [numpy.average(initpoints[0,g1]),numpy.average(initpoints[1,g1])]
                        m1 = numpy.arctan2((destination1[1] - GCM1[1]),(destination1[0] - GCM1[0]))                    
                        sub_dest1[0] = numpy.average(initpoints[0,g1]) + rs_scale*rs*numpy.cos(m1)
                        sub_dest1[1] = numpy.average(initpoints[1,g1]) + rs_scale*rs*numpy.sin(m1)
                        print("sub_dest1: " , sub_dest1)
                        orient1 = 1;
                    if(orient1 == 1 and leader_front1 == 0):
                        #print("OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")
                        sheepmovements(initpoints,((len(g1))),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode1,destination1,p,swarm_split)
                        S0,a = moveSheperd(Sh[0],sub_dest1,1,1)
                        S2,a = moveSheperd(Sh[2],sub_dest1,1,1)
                        sub_D1 = numpy.linalg.norm(initpoints.T - S0,axis=1)
                        sub_D11 = numpy.linalg.norm(initpoints.T - S2,axis=1)
                        sub_n1 = numpy.where(sub_D1 < rs)
                        sub_n11 = numpy.where(sub_D11 < rs)
                        if(numpy.size(sub_n1) > 0):
                            print("avg: " , numpy.average(initpoints[:,g1]))
                            S0,a = moveSheperd(S0,numpy.asarray(initpoints[:,sub_n1[0][0]]).reshape(1,2),1.25,-1)    
                        if(numpy.size(sub_n11) > 0): #numpy.reshape(initpoints[:,sub_n1[0][0]])                            
                            S2,a = moveSheperd(S2,numpy.asarray(initpoints[:,sub_n11[0][0]]).reshape(1,2),1.25,-1)
                        if(numpy.linalg.norm(S0 - S2) < 7.5):
                        	S2,a = moveSheperd(S2,S0,1.15,-1)        

                    if(orient1 == 1 and leader_front1 == 1):    
                        print("11111&&&&&&&&&&&&&&&&&&&&&&&&&&&&7&&&&&&&&&")
                        print("leader_front1:" , leader_front1)
                        temp_dist1 = numpy.linalg.norm(initpoints.T-Sh[0],axis=1)
                        sort_temp_dist1 = numpy.sort(temp_dist1)
                        print("sort_temp_dist1[0] " , sort_temp_dist1[0])
                        if(sort_temp_dist1[0] > 5.0):
                            S0,a = moveSheperd(Sh[0],GCM1,0.5,1)
                        else:
                            print("LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL")
                            leader_front1 = 2 
                            print("leader_front1 in else " , leader_front1)
                        temp_dist1 = numpy.linalg.norm(initpoints.T-Sh[2],axis=1)
                        sort_temp_dist1 = numpy.sort(temp_dist1)
                        if(sort_temp_dist1[0] > 5.0):
                            S2,a = moveSheperd(Sh[2],GCM1,0.5,1)
                        if(numpy.linalg.norm(S0 - S2) < 7.5):
                        	S2,a = moveSheperd(S2,S0,1.15,-1)        

                    if(leader_front1 == 2):                    
                        leader_on1 = 1;
                        sheepmovements(initpoints,((len(g1))),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode1,destination1,p,leader_on1)
                        S0 = leadermovement(Sh[0].reshape(1,2),initpoints[:,g1],destination1,rs,rrep,step)
                        S2 = leadermovement(Sh[2].reshape(1,2),initpoints[:,g1],destination1,rs,rrep,step)
                    ...
                    if(entered1 == 0):        
                        dest1 = [0,0];
                        dest1[0] = destination1[0] + 5.0*numpy.cos(m1)
                        dest1[1] = destination1[1] + 5.0*numpy.sin(m1) #  - sin() as alpha1 is negative 
                ...
                if(len(g2) > 0.1*(numpy.size(grp2)) and eating2 == 0):
                    print("GRP2")
                    #sheepmovements(initpoints,((len(g2))),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode2,destination2,p,swarm_split)
                    
                    if(orient2 == 0): #+++++++++++++++++++++++++++++++++++++++++++++++++++++++
                        sheepmovements(initpoints,((len(g2))),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode2,destination2,p,swarm_split)
                        GCM2 = [numpy.average(initpoints[0,g2]),numpy.average(initpoints[1,g2])]
                        m2 = numpy.arctan2((destination2[1] - GCM2[1]),(destination2[0] - GCM2[0]))
                        sub_dest2[0] = numpy.average(initpoints[0,g2]) + rs_scale*rs*numpy.cos(m2)
                        sub_dest2[1] = numpy.average(initpoints[1,g2]) + rs_scale*rs*numpy.sin(m2)
                        print("sub_dest2: " , sub_dest2)    
                        orient2 = 1;
                    if(orient2 == 1 and leader_front2 == 0):
                        sheepmovements(initpoints,((len(g2))),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode2,destination2,p,swarm_split)
                        S1,a = moveSheperd(Sh[1],sub_dest2,1,1)
                        print("MOVEEEEEEEEEEEEE")
                        sub_D2 = numpy.linalg.norm(initpoints.T - S1,axis=1)
                        sub_n2 = numpy.where(sub_D2 <= rs)
                        if(numpy.size(sub_n2) > 0):
                            #print("avg: " , numpy.average(initpoints[:,g1]))
                            S1,a = moveSheperd(S1,numpy.asarray(initpoints[:,sub_n2[0][0]]).reshape(1,2),1.25,-1)                                    

                        S3,a = moveSheperd(Sh[3],sub_dest2,1,1)
                        print("MOVEEEEEEEEEEEEE")
                        sub_D2 = numpy.linalg.norm(initpoints.T - S3,axis=1)
                        sub_n2 = numpy.where(sub_D2 <= rs)
                        if(numpy.size(sub_n2) > 0):
                            #print("avg: " , numpy.average(initpoints[:,g1]))
                            S3,a = moveSheperd(S3,numpy.asarray(initpoints[:,sub_n2[0][0]]).reshape(1,2),1.25,-1)
                        if(numpy.linalg.norm(S1 - S3) < 7.5):
                        	S3,a = moveSheperd(S3,S1,1.15,-1)


                    if(orient2 == 1 and leader_front2 == 1):
                        print("TOWARDS AGENTSSS")
                        temp_dist2 = numpy.linalg.norm(initpoints.T-Sh[1],axis=1)
                        sort_temp_dist2 = numpy.sort(temp_dist2)
                        if(sort_temp_dist2[0] > 5.0):
                            S1,a = moveSheperd(Sh[1],GCM2,0.5,1)
                        else:
                            leader_front2 = 2
                        temp_dist2 = numpy.linalg.norm(initpoints.T-Sh[3],axis=1)
                        sort_temp_dist2 = numpy.sort(temp_dist2)
                        if(sort_temp_dist2[0] > 5.0):
                            S3,a = moveSheperd(Sh[3],GCM2,0.5,1)
                        if(numpy.linalg.norm(S1 - S3) < 7.5):
                        	S3,a = moveSheperd(S3,S1,1.15,-1)

                    if(leader_front2 == 2):    
                        leader_on2 = 1
                        print("Sh1:", Sh[1] , " GCM2: " , numpy.average(initpoints[0,grp2]), " " , numpy.average(initpoints[1,g2]))
                        sheepmovements(initpoints,((len(g2))),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode2,destination2,p,leader_on2)
                        S1 = leadermovement(Sh[1].reshape(1,2),initpoints[:,g2],destination2,rs,rrep,step)
                        S3 = leadermovement(Sh[3].reshape(1,2),initpoints[:,g2],destination2,rs,rrep,step)
                    ...
                    if(entered2 == 0):
                        dest2 = [0,0];
                        dest2[0] = destination2[0] + 5.0*numpy.cos(m2)
                        dest2[1] = destination2[1] + 5.0*numpy.sin(m2)
                ...
                if(out1 == 1 and out2 == 1 and pos_update1 == 1 and pos_update2 == 1):
                    print("|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||")
                    outthreshold = []        
                    for i in range(N):
                    ...
                    #     #out3 = 1;
                    if(len(g1) > ceil(0.1*len(grp1)) and in1 == 0):
                        leader_on1 = 1
                        sheepmovements(initpoints,0.9*len(g1),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode1,destination,p,leader_on1)
                        #S0,mode1,I1 = sheperdMovement(Sh[0].reshape(1,2),initpoints[0][g1],initpoints[1][g1],ra,Pd,Pc,destination,f,mindisShep,rhoash,alpha,ang,mode1,nin,ds,rs,method,predmasterslave,decrad,q,I1)      
                        S0 = leadermovement(Sh[0].reshape(1,2),initpoints[:,g1],destination,rs,rrep,step)                
                        S2 = leadermovement(Sh[2].reshape(1,2),initpoints[:,g1],destination,rs,rrep,step)                
                    else:
                        in1 = 1;   

                    if(len(g2) > ceil(0.1*len(grp2)) and in2 == 0):
                        leader_on2 = 1
                        sheepmovements(initpoints,0.9*len(g2),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode2,destination,p,leader_on2)
                        #S1,mode2,I2 = sheperdMovement(Sh[1].reshape(1,2),initpoints[0][g2],initpoints[1][g2],ra,Pd,Pc,destination,f,mindisShep,rhoash,alpha,ang,mode2,nin,ds,rs,method,predmasterslave,decrad,q,I2)                      
                        S1 = leadermovement(Sh[1].reshape(1,2),initpoints[:,g2],destination,rs,rrep,step)
                        S3 = leadermovement(Sh[3].reshape(1,2),initpoints[:,g2],destination,rs,rrep,step)
                    else:    
                        in2 = 1;
                    
                    if(in1 == 1 and in2 == 1):
                        swarm_split = 0; #joined #start feeeding
                        eating3 = 1;
                        #rs = 1;
                        destination = [0,0]
                        print("=======================================================================")
            ...
            if(eating1 == 1 and out1 == 0):
                M.markers[N1+nos].scale.x = M.markers[N1+nos].scale.x - size_dec
                M.markers[N1+nos].scale.y = M.markers[N1+nos].scale.y - size_dec
            ...
            if(eating1 == 1 and pos_update1 == 0):    
                #if(sqrt(pow(dest1[0] - Sh[0][0],2) + pow(dest1[1] - Sh[0][1],2)) > 3.0):
                dr1 = numpy.linalg.norm(Sh[0] - dest1)
                if(dr1 > 5):                
                    ...
                    S0 , a = moveSheperd(S0,numpy.asarray(dest1).reshape(1,2),1,1)
                    S2 , a = moveSheperd(S2,numpy.asarray(dest1).reshape(1,2),1,1)
                    D1=numpy.linalg.norm(initpoints.T-S0,axis=1)
                    D11=numpy.linalg.norm(initpoints.T-S2,axis=1)
                    if(numpy.where(D1 < rs)):
                        S0, a = moveSheperd(S0,numpy.asarray(destination1).reshape(1,2),1.25,-1)
                    if(numpy.where(D11 < rs)):
                        S2, a = moveSheperd(S2,numpy.asarray(destination1).reshape(1,2),1.25,-1)                        
                    if(numpy.linalg.norm(S2 - S0) < 5):
                    	S2,a = moveSheperd(S2,S0,1.15,-1)    
                    
                    Sh[0] = S0[0]
                    Sh[2] = S2[0]	
                    #cur_angle1 = cur_angle1 - pi/18 #theta1
                    #S0[0][0] = new_pos1[0]
                    #S0[0][1] = new_pos1[1]
                    #S0 , a1 = moveSheperd(Sh[0],numpy.asarray(new_pos1).reshape(1,2),1,1)
                    # print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                    # print("m1: " , m1 , " cur_angle1 " , cur_angle1)
                else:
                    pos_update1 = 1;    
            ...
            if(eating2 == 1 and out2 == 0):
                M.markers[N1+nos+1].scale.x = M.markers[N1+nos+1].scale.x - size_dec
                M.markers[N1+nos+1].scale.y = M.markers[N1+nos+1].scale.y - size_dec
            ...
            if(eating2 == 1 and pos_update2 == 0):        
                #if(sqrt(pow(dest2[0] - Sh[1][0],2) + pow(dest2[1] - Sh[1][1],2)) > 3.0):
                dr2 = numpy.linalg.norm(Sh[1] - dest2)
                if(dr2 > 5):
                    ...
                    S3 , a = moveSheperd(S3,numpy.asarray(dest2).reshape(1,2),1,1)
                    D2=numpy.linalg.norm(initpoints.T-S1,axis=1)
                    if(numpy.where(D2 < rs)):
                        S1, a = moveSheperd(S1,numpy.asarray(destination2).reshape(1,2),1.25,-1.)
                    D22=numpy.linalg.norm(initpoints.T-S3,axis=1)
                    if(numpy.where(D22 < rs)):
                        S3, a = moveSheperd(S3,numpy.asarray(destination2).reshape(1,2),1.25,-1.)
                    if(numpy.linalg.norm(S3 - S1) < 5):
                    	S3,a = moveSheperd(S3,S1 , 1.15 , -1)    
                    #new_pos2[0] = destination2[0] + (d2+dr)*numpy.cos(cur_angle2 - pi/36)
                    #new_pos2[1] = destination2[1] + (d2+dr)*numpy.sin(cur_angle2 - pi/36)
                    #cur_angle2 = cur_angle2 - pi/18  #theta2
                    #S1 , a1 = moveSheperd(Sh[1],numpy.asarray(new_pos2).reshape(1,2),1,1)
                    # S1[0][0] = new_pos2[0]
                    # S1[0][1] = new_pos2[1]
                    #print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                    #print("m2: " , m2 , " cur_angle2 " , cur_angle2)
            ...
            if(leader_front1 == 0 and orient1 == 1):
                print("LEADER1: " , Sh[0] , " " , "sub_dest1:",sub_dest1)
                print("GCM1: " ,GCM1)
                #if(numpy.linalg.norm(sub_dest1 - Sh[0],axis = 0) <= 2.0):
                dist_from_subdest1 = numpy.linalg.norm(sub_dest1 - Sh[0])
                #if(Sh[0][0] < GCM1[0] and Sh[0][1] < GCM1[1]):
                if(dist_from_subdest1 < 5):
                    leader_front1 = 1;

            if(leader_front2 == 0 and orient1 == 1):
                print("LEADER2: " , Sh[1] , " " , "sub_dest2:",sub_dest2)
                print("GCM2: " ,GCM2)
                dist_from_subdest2 = numpy.linalg.norm(sub_dest2 - Sh[1])                
                #if(numpy.linalg.norm(sub_dest2 - Sh[1],axis = 0) <= 2.0):
                if(dist_from_subdest2 < 5):
                    leader_front2 = 1;
            ...
            if(Sh[0][1] <= Sh[1][1] and enter_once == 0):
                ...
                temp_grp1 = copy.deepcopy(grp1)
                temp_grp2 = copy.deepcopy(grp2)
            ...
            if(in1 == 1 and in2 == 1 and eating3 == 1): #START Feeding
                dT3 = numpy.linalg.norm(initpoints.T - destination,axis = 1);
                near_dest = numpy.where(dT3 < 15)
                M.markers[N1+nos+2].scale.x = M.markers[N1+nos+2].scale.x - size_dec
                M.markers[N1+nos+2].scale.y = M.markers[N1+nos+2].scale.y - size_dec
                if(M.markers[N1+nos+2].scale.y < 0.2):
                    eaten3 = 1;
            ...
            # if(enter_once == 1):
            #     for i in grp1:
            #         d = numpy.linalg.norm(initpoints[:,i] - initpoints[:,grp2].T,axis=1)
            #         c = numpy.where(d < 5.0)
            #         if(numpy.size(c) > 0):
            #             flag = 1;
            #             break;
            # if(flag == 1):
            #     break        
            # sh_near_dest = numpy.linalg.norm(Sh - destination,axis=1)
            # sh_count = numpy.where(sh_near_dest < threshold/3.0)
            # if(numpy.size(sh_count) == 2):
            #     break;
            ...
            if(count > 1500):
                ...
                if(out2 != 1):
                    ...
                    #    task[0][mc1] = task[0][mc1] + 2
                if(eaten3 != 1 and out1 == 1 and out2 == 1):
        ...
        print( agent_sep)
        print("++++++++++++++++++++++++++++++++++++++++++")
        print("tasks not not complete: " , task)
