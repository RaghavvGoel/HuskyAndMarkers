...
def main():
    rospy.init_node('sheep_sheperd200', anonymous=True)
        ...
        while not rospy.is_shutdown():
            ...
            if(toggle == 0):
                ...
                if(len(g1) >= 0.1*(numpy.size(grp1)) and eating1 == 0):
                    ...
                    S0,mode1,I1 = sheperdMovement(numpy.asarray([Sh[0],Sh[2]]).reshape(2,2),initpoints[0][g1],initpoints[1][g1],ra,Pd,Pc,destination1,f,mindisShep,rhoash,alpha,ang,mode1,nin,ds,rs,method1,predmasterslave1,decrad,q,I1)
                else:
                ...
                if(len(g2) >= 0.1*(numpy.size(grp2)) and eating2 == 0):
                    ...
                    Sh1,mode2,I2 = sheperdMovement(numpy.asarray([Sh[1],Sh[3]]).reshape(2,2),initpoints[0][g2],initpoints[1][g2],ra,Pd,Pc,destination2,f,mindisShep,rhoash,alpha,ang,mode2,nin,ds,rs,method2,predmasterslave2,decrad,q,I2)
                else:
                    #out2 = 1;
                    eating2 = 1;
                ...
                if(out1 == 1 and out2 == 1 ): #and pos_update1 == 1 and pos_update2 == 1
                    ...
                    if(len(g1) > ceil(0.1*len(grp1)) and in1 == 0 and too_close == 0):
                        sheepmovements(initpoints,n,c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode1,destination,p,swarm_split)
                        S0,mode1,I1 = sheperdMovement(numpy.asarray([Sh[0],Sh[2]]).reshape(2,2),initpoints[0][g1],initpoints[1][g1],ra,Pd,Pc,destination,f,mindisShep,rhoash,alpha,ang,mode1,nin,ds,rs,method1,predmasterslave1,decrad,q,I1)      
                    ...
                    if(len(g2) > ceil(0.1*len(grp2)) and in2 == 0 and too_close == 0):#if too_close becomes 1, then in2 will become 1
                        ...
                        Sh1,mode2,I2 = sheperdMovement(numpy.asarray([Sh[1],Sh[3]]).reshape(2,2),initpoints[0][g2],initpoints[1][g2],ra,Pd,Pc,destination,f,mindisShep,rhoash,alpha,ang,mode2,nin,ds,rs,method2,predmasterslave2,decrad,q,I2)                      
            ...
            if(count > 2000):
                ...
                if(eating1 == 0 or  M.markers[N+nos].scale.x > 0.2):
                    task[0][mc1] = 1;
                if(eating2 == 0 or M.markers[N+nos+1].scale.x > 0.2):
