...
#                         [ 22.,  67.,  66.,  39.,18.,  28., -47.,  18.]]))
    ...
    for mc1 in range(MC):
        ...
        while not rospy.is_shutdown():
            ...
            #     ind_l = numpy.random.choice(N,2) # choosing 
            ...
            if(eaten1 == 0 and reached1 == 0):
                ...
                m1 = numpy.arctan2(destination1[0][1] - gcm[1],destination1[0][0] - gcm[0])
                pos1 = [0.0,0.0]
                pos1[0] = gcm[0] - (r-dr)*numpy.cos(m1)
                pos1[1] = gcm[1] - (r-dr)*numpy.sin(m1)
                print("pos1: " , pos1)
                S0,a = moveSheperd(Sh[0],pos1,sp,1)
            elif(reached1 == 1 and (eaten1 == 0 or eaten2 == 0)):
                m1 = numpy.arctan2(destination2[0][1] - gcm[1],destination2[0][0] - gcm[0])
                pos1 = [0.0,0.0]
                pos1[0] = gcm[0] - 2*r*numpy.cos(m1)
                pos1[1] = gcm[1] - 2*r*numpy.sin(m1)                    
                S0,a = moveSheperd(Sh[0],pos1,sp,1)
                n1 = numpy.linalg.norm(initpoints.T - S0)
                if(numpy.size(n1) > 0):
                    S0,a = moveSheperd(S0,gcm1,sp/2,-1) #numpy.asarray(gcm2).reshape(1,2)                   
                print("pos1: " , pos1)                        
            elif(eaten1 == 1 and reached1 == 1 and reached2 == 0):                
                ...
                print("pos1: " , pos1)
            elif(reached2 == 1 and eaten2 == 0):
                m1 = numpy.arctan2(destination[0][1] - gcm[1],destination[0][0] - gcm[0])
                pos1 = [0.0,0.0]
                pos1[0] = gcm[0] - 2*r*numpy.cos(m1)
                pos1[1] = gcm[1] - 2*r*numpy.sin(m1)                    
                S0,a = moveSheperd(Sh[0],pos1,sp,1)
                n1 = numpy.linalg.norm(initpoints.T - S0)
                if(numpy.size(n1) > 0):
                    S0,a = moveSheperd(S0,gcm,sp/2,-1) #numpy.asarray(gcm2).reshape(1,2)                   
                print("pos1: " , pos1)                                            
            elif(eaten2 == 1 and reached3 == 0):                
                print("reached 3")
                m1 = numpy.arctan2(destination[0][1] - gcm[1],destination[0][0] - gcm[0])
                #sp1 = 0.004
                pos1 = [0.0,0.0]                    
            ...
            if(eaten1 == 0 and reached1 == 0):
                #S0,a = moveSheperd(initpoints[:,leaderidx[0]],copy.deepcopy(destination1),sp,1)
                #pos1 is the point on the line of destination1 and gcm1
                m2 = numpy.arctan2(destination1[0][1] - gcm[1],destination1[0][0] - gcm[0])
                pos2 = [0.0,0.0]
                pos2[0] = gcm[0] - (r-dr)*numpy.cos(m2)
                pos2[1] = gcm[1] - (r-dr)*numpy.sin(m2)
                print("pos1: " , pos2)
                S1,a = moveSheperd(Sh[1],pos2,sp,1)
            elif(reached1 == 1 and (eaten1 == 0 or eaten2 == 0)):
                m2 = numpy.arctan2(destination2[0][1] - gcm[1],destination2[0][0] - gcm[0])
                pos2 = [0.0,0.0]
                pos2[0] = gcm[0] - 2*r*numpy.cos(m2)
                pos2[1] = gcm[1] - 2*r*numpy.sin(m2)                    
                S1,a = moveSheperd(Sh[1],pos1,sp,1)
                n2 = numpy.linalg.norm(initpoints.T - S1)
                if(numpy.size(n2) > 0):
                    S1,a = moveSheperd(S1,gcm1,sp/2,-1) #numpy.asarray(gcm2).reshape(1,2)                   
                print("pos1: " , pos1)                        
            elif(eaten1 == 1 and reached1 == 1 and reached2 == 0):                
                #S0,a = moveSheperd(initpoints[:,leaderidx[0]],copy.deepcopy(destination),sp,1)   
                m2 = numpy.arctan2(destination2[0][1] - gcm[1],destination2[0][0] - gcm[0])
                ...
                m2 = numpy.arctan2(destination[0][1] - gcm[1],destination[0][0] - gcm[0])
                pos2 = [0.0,0.0]
                pos2[0] = gcm[0] - 2*r*numpy.cos(m2)
                pos2[1] = gcm[1] - 2*r*numpy.sin(m2)                    
                S1,a = moveSheperd(Sh[1],pos2,sp,1)
                ...
                #sp1 = 0.004
                pos2 = [0.0,0.0]                    
                pos2[0] = gcm[0] - (r-dr)*numpy.cos(m1)
                pos2[1] = gcm[1] - (r-dr)*numpy.sin(m1)                    
                S1,a = moveSheperd(Sh[1],pos2,sp1,1)
                print("pos1: " , pos2)                
            ...
            if(eaten1 == 1 and eaten2 == 1):
                g1 = numpy.intersect1d(grp1[0],near_dest[0])    
                if(numpy.size(g1) >= 0.9*numpy.size(grp1)):
                    g1_reached3 = 1;

                g2 = numpy.intersect1d(grp2[0],near_dest[0])    
                if(numpy.size(g2) >= 0.9*numpy.size(grp2)):
                    g2_reached3 = 1;
            


            if(numpy.size(near_dest1) > 0.9*N and eaten1 == 0):
                M.markers[N+nos].scale.x = M.markers[N+nos].scale.x - size_dec/N*(numpy.size(near_dest1)) #*(numpy.size(near_dest2))         
                M.markers[N+nos].scale.y = M.markers[N+nos].scale.y - size_dec/N*(numpy.size(near_dest1))          #*(numpy.size(near_dest2))
                if(M.markers[N+nos].scale.y < 0.2):
                    eaten1 = 1;                        
            if(numpy.size(near_dest2) > 0.9*N and eaten2 == 0):
                M.markers[N+nos+1].scale.x = M.markers[N+nos+1].scale.x - size_dec/N*(numpy.size(near_dest2)) #*(numpy.size(near_dest2))         
                M.markers[N+nos+1].scale.y = M.markers[N+nos+1].scale.y - size_dec/N*(numpy.size(near_dest2))          #*(numpy.size(near_dest2))
                if(M.markers[N+nos+1].scale.y < 0.2):
                    eaten2 = 1;
    ...
    print("not_complted: " , not_complted)
    print("Final Destinations: " )
    # print("dest1: " , dest1_arr)
    # print("dest2: " , dest2_arr)
    # print("dest: " , dest_arr)    
