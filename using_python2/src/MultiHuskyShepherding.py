...
def sheperdMovement( Sh,X,Y,ra,Pd,Pc,destination,f,mindisShep,rhoash,alpha,ang,mode,nin,ds,rs,method,predmasterslave,decrad,q,I):
    ...
    if(numpy.shape(Sh)[0] == 1):
        ...
        idx = []
        for i in range(len(X)):
            DGCM.append(numpy.sqrt(numpy.square(X1[i] - XGCM) + numpy.square(Y1[i] - YGCM)))
            if(DGCM[i] > f):
                idx.append(i)                                       
        if(len(idx) <= len(X) and mode == 1):
            ...
            alpha = 1
            ang = 0 
            Sh = surroundEffect(Sh,X1,Y1,ra,Pd,destination,q,mindisShep,rhoash,rs);
...
#Final Destinations: 
#('dest1: ',)
    destination1x = [0.0 , -71. ,-108. ,-140. , -63. , -82. , -78. ,-116. , -63. , -96., -119. , -71.,  -109. , -62. ,-105. , -91. ,-142. , -68. ,-128. ,-133. ,-136. , -80. ,-128. , -59.,
  -130.]
...
#('dest2: ',)
    destination2x = [ 0.0 , 198. , 169. , 218.,  179. , 171.,  223.,  187.,  240. , 212. , 232. , 239.,
   196. , 248. , 176.  ,243. , 157. , 220. , 179. , 151. , 197. , 196. , 164. , 240.,
   230.]
    destination2y = [ 40.0 ,-107., -228.,  234., -157., -223. ,-209. ,-141. , 145. , 138. ,  37. ,-162.,
...
#('dest: ',)
   ...
   142.]
    ...
    for mc1 in range(MC):            
        ...
        N1 = copy.deepcopy(N)
        ra=4.0  # distance for self repulsion
        mindisShep = 6
        n=int(numpy.ceil(0.4*N1))
        #n = 15
        Pd = ra*log(N)
        Pc = ra
        threshold = 10.0
        ...
        while not rospy.is_shutdown():
            ...
            if(toggle == 0):
                ...
                if(out1 == 0 or out2 == 0):
                    ...
                    if(len(g1) >= 0.1*(numpy.size(grp1)) and eating1 == 0):
                        ...
                        S0,mode1,I1 = sheperdMovement(numpy.asarray([Sh[0]]).reshape(nos/2,2),initpoints[0][g1],initpoints[1][g1],ra,Pd1,Pc,destination1,f1,mindisShep,rhoash,alpha,ang,mode1,nin,ds,rs,method1,predmasterslave1,decrad,q,I1)
                    else:
                    ...
                    if(len(g2) >= 0.1*(numpy.size(grp2)) and eating2 == 0):
                        ...
                        S1,mode2,I2 = sheperdMovement(numpy.asarray([Sh[1]]).reshape(nos/2,2),initpoints[0][g2],initpoints[1][g2],ra,Pd2,Pc,destination2,f2,mindisShep,rhoash,alpha,ang,mode2,nin,ds,rs,method2,predmasterslave2,decrad,q,I2)
                    else:
                        #out2 = 1;
                        eating2 = 1;
                        ...
                        if not (Sh[1][0] >= dest2[0] and Sh[1][1] >= dest2[1]):
                            ...
                            S10 , a = moveSheperd(S1[0],numpy.asarray(dest2).reshape(1,2),1,1)
                ...
                if(out1 == 1 and out2 == 1 and (in1 == 0 or in2 == 0)): #and pos_update1 == 1 and pos_update2 == 1
                    ...
                    if(len(g1) > ceil(0.1*len(grp1)) and in1 == 0 and too_close == 0):
                        ...
                        S0,mode1,I1 = sheperdMovement(numpy.asarray([Sh[0]]).reshape(nos/2,2),initpoints[0][g1],initpoints[1][g1],ra,Pd1,Pc,destination,f1,mindisShep,rhoash,alpha,ang,mode1,nin,ds,rs,method1,predmasterslave1,decrad,q,I1)      
                    ...
                    if(len(g2) > ceil(0.1*len(grp2)) and in2 == 0 and too_close == 0):#if too_close becomes 1, then in2 will become 1
                        ...
                        S1,mode2,I2 = sheperdMovement(numpy.asarray([Sh[1]]).reshape(nos/2,2),initpoints[0][g2],initpoints[1][g2],ra,Pd2,Pc,destination,f2,mindisShep,rhoash,alpha,ang,mode2,nin,ds,rs,method2,predmasterslave2,decrad,q,I2)                      
            ...
            for i in range(N):
                ...
                delta_theta[i] = change_in_heading[i] - Yaw[i]
                dist_left_to_cover[i] = numpy.sqrt(pow(huskyX[i] - initpoints[0][i],2) + pow(huskyY[i] - initpoints[1][i],2))   
                if(dist_left_to_cover[i] > 1.0):
                    dist_left_to_cover[i] = 1.0
                ...
                #print("dist left:", dist_left_to_cover[i])
                if(i in grp1 and abs(delta_theta[i]) > pi): #a safety precaution for Sh moving towards the left
                    delta_theta[i] = (-1)*(numpy.sign(delta_theta[i]))*(abs(delta_theta[i])%(2*pi))    
            ...
            for i in range(nos):    
                ...
                if(toggle == 1):
                    change_in_heading_shepherd[i] = numpy.arctan2(Sh[i][1] - temp_Sh[i][1] , Sh[i][0] - temp_Sh[i][0])
                    delta_theta_shepherd[i] = change_in_heading_shepherd[i] - Yaw_pred[i];
                    dist_from_sheep = numpy.linalg.norm(initpoints.T - Sh[i], axis = 1)
                    sheep_closeby = numpy.where(dist_from_sheep < ra)
                    if(numpy.size(sheep_closeby) > 0):
                        sheperd_speed[i] = 0.5
                    else:
                ...
                # Sh10 = [Sh[1][0],Sh[1][1]]
                # Sh11 = [Sh[3][0],Sh[3][1]]                   
                if(i == 0 and abs(delta_theta_shepherd[i]) > pi): #a safety precaution for Sh moving towards the left
                    delta_theta_shepherd[i] = (-1)*(numpy.sign(delta_theta_shepherd[i]))*(abs(delta_theta_shepherd[i])%(2*pi))
                H_pred[i].linear.x = sheperd_speed[i]
                H_pred[i].angular.z = k_pred*delta_theta_shepherd[i]                
            ...
            if(abs(Sh[0][1] - Sh[1][1]) < 6 and enter_once == 0 and husky_predY[0] != 0 and husky_predY[1] != 0):
                switch = 0;
                toggle = 1;
                #crossed_swarm = 1;
                enter_once = 1;
