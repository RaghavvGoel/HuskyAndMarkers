...
def main():
    rospy.init_node('sheep_sheperd_split2', anonymous=True)
    ...
    for mc2 in range(MC):
        for mc in range(5):
            ...
            while not rospy.is_shutdown():
                ...
                # print("Sh[0]: " , Sh[0])
                #print("D1: " , D1)
                # if(nos > 1 and abs(Sh[0][1] - Sh[1][1]) <= 0):
                #     s = 0.0    
                if((n2 > 0 or n3 > 0)):
                    s = 0.15 + mc2*0.05
                    #################################################################
    ...
    for i in range(MC):
        print("speed = " , 0.35 + i*0.05 , "time: " , numpy.sum(time[i])/5 , "  ratio: " , numpy.sum(ratio[i])/(N*5 - numpy.sum(ratio[i])))
