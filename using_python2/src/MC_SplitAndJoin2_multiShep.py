        ...
        while not rospy.is_shutdown():
            ...
            if(toggle == 0):
                ...
                if(len(g1) >= 0.1*(numpy.size(grp1)) and eating1 == 0):
                    ...
                    S0,mode1,I1 = sheperdMovement(numpy.asarray([Sh[0],Sh[2]]).reshape(2,2),initpoints[0][g1],initpoints[1][g1],ra,Pd,Pc,destination1,f,mindisShep,rhoash,alpha,ang,mode1,nin,ds,rs,method,predmasterslave,decrad,q,I1)
                else:
                ...
                if(len(g2) >= 0.1*(numpy.size(grp2)) and eating2 == 0):
                    ...
                    Sh1,mode2,I2 = sheperdMovement(numpy.asarray([Sh[1],Sh[3]]).reshape(2,2),initpoints[0][g2],initpoints[1][g2],ra,Pd,Pc,destination2,f,mindisShep,rhoash,alpha,ang,mode2,nin,ds,rs,method,predmasterslave,decrad,q,I2)
                else:
                    #out2 = 1;
                    eating2 = 1;
                ...
                if(out1 == 1 and out2 == 1 ): #and pos_update1 == 1 and pos_update2 == 1
                    ...
                    for i in range(N):
                        ...
                        if(sqrt(pow(initpoints[0][i] - destination[0],2) + pow(initpoints[1][i] - destination[1],2)) > (threshold)): #making threshold 20, so that joining happens earleir                    
                            outthreshold.append(i)
                    ...
                    if(len(g1) > ceil(0.1*len(grp1)) and in1 == 0 and too_close == 0):
                        sheepmovements(initpoints,0.9*len(g1),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode1,destination,p,swarm_split)
                        S0,mode1,I1 = sheperdMovement(numpy.asarray([Sh[0],Sh[2]]).reshape(2,2),initpoints[0][g1],initpoints[1][g1],ra,Pd,Pc,destination,f,mindisShep,rhoash,alpha,ang,mode1,nin,ds,rs,method,predmasterslave,decrad,q,I1)      
                    ...
                    if(len(g2) > ceil(0.1*len(grp2)) and in2 == 0 and too_close == 0):#if too_close becomes 1, then in2 will become 1
                        sheepmovements(initpoints,0.9*len(g2),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode2,destination,p,swarm_split)
                        Sh1,mode2,I2 = sheperdMovement(numpy.asarray([Sh[1],Sh[3]]).reshape(2,2),initpoints[0][g2],initpoints[1][g2],ra,Pd,Pc,destination,f,mindisShep,rhoash,alpha,ang,mode2,nin,ds,rs,method,predmasterslave,decrad,q,I2)                      
                    else:    
                        in2 = 1;
            ...
            if(eating1 == 1 and out1 == 0): # 
                ...
                M.markers[N1+nos].scale.x = M.markers[N1+nos].scale.x - (0.2/len(grp1))*(in_dest1)
                M.markers[N1+nos].scale.y = M.markers[N1+nos].scale.y - (0.2/len(grp1))*(in_dest1)
            ...
            if(eating1 == 1 and pos_update1 == 0 and (out2 == 0 or out1 == 0)):    
                ...
                if not (Sh[0][0] <= dest1[0] and Sh[0][1] >= dest1[1]):
                    ...
                    S00 , a = moveSheperd(Sh[0],numpy.asarray(dest1).reshape(1,2),1,1)
                    S2 , a = moveSheperd(Sh[2],numpy.asarray(dest1).reshape(1,2),1,1)
                    D1=numpy.linalg.norm(initpoints.T-S00,axis=1)
                    if(numpy.where(D1 < rs)):
                        S00, a = moveSheperd(S00,numpy.asarray(destination1).reshape(1,2),1,-1)

                    D1=numpy.linalg.norm(initpoints.T-S2,axis=1)
                    if(numpy.where(D1 < rs)):
                        S2, a = moveSheperd(S2,numpy.asarray(destination1).reshape(1,2),1,-1)  
                    if(numpy.linalg.norm(S00 - S2) < 5):
                    	S2,a = moveSheperd(S2,S00,1.15,-1)    
            ...
            if(eating2 == 1 and out2 == 0 ): #
                ...
                M.markers[N1+nos+1].scale.x = M.markers[N1+nos+1].scale.x - (0.2/len(grp2))*((in_dest2))
                M.markers[N1+nos+1].scale.y = M.markers[N1+nos+1].scale.y - (0.2/len(grp2))*((in_dest2))
            ...
            if(eating2 == 1 and pos_update2 == 0 and (out2 == 0 or out1 == 0)):        
                ...
                if not (Sh[1][0] >= dest2[0] and Sh[1][1] >= dest2[1]):
                    ...
                    S11 , a = moveSheperd(Sh[1],numpy.asarray(dest2).reshape(1,2),1,1)
                    S3 , a = moveSheperd(Sh[3],numpy.asarray(dest2).reshape(1,2),1,1)
                    D2=numpy.linalg.norm(initpoints.T-S11,axis=1)
                    if(numpy.where(D2 < rs)):
                        S11, a = moveSheperd(S11,numpy.asarray(destination2).reshape(1,2),1,-1)

                    D2=numpy.linalg.norm(initpoints.T-S3,axis=1)
                    if(numpy.where(D2 < rs)):
                        S3, a = moveSheperd(S3,numpy.asarray(destination2).reshape(1,2),1,-1)                        
                    if(numpy.linalg.norm(S11 - S3) < 5):
                    	S3,a = moveSheperd(S3,S11,1.15,-1)    
            ...
            outthreshold = numpy.where(dist_from_dest > 15)
            inthreshold = numpy.where(dist_from_dest <= 15)
            if(numpy.size(inthreshold) > 0.9*N and out1 == 1 and out2 == 1 and eaten3 == 0):
                #START FEEDING
                print("PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP")
                print("inthreshold: " , numpy.size(inthreshold) , " " , 0.2/(0.9*N) , " " , 0.2/(0.9*N)*(numpy.size(inthreshold)))
                M.markers[N+nos+nod-1].scale.x = M.markers[N+nos+nod-1].scale.x - 0.2/(0.9*N)*(numpy.size(inthreshold))
                M.markers[N+nos+nod-1].scale.y = M.markers[N+nos+nod-1].scale.y - 0.2/(0.9*N)*(numpy.size(inthreshold))
