...
global husky1_x, husky1_y, husky2_x, husky2_y #not necessary to use global keyword here but necessary in local functions
husky_xy = numpy.zeros((4,2))
husky_orientation = numpy.zeros((4,1))
I = []
...
def position_husky1(data1):
    ...
    husky_xy[0][0] = data1.pose.pose.position.x
    husky_xy[0][1] = data1.pose.pose.position.y
    husky_orientation[0][0] = data1.pose.pose.orientation.z

def position_husky2(data2):
    global husky_xy,husky_orientation
    husky_xy[1][0] = data2.pose.pose.position.x
    husky_xy[1][1] = data2.pose.pose.position.y
    husky_orientation[1][0] = data2.pose.pose.orientation.z

def position_husky3(data3):
    #global husky3_y, husky3_x
    global husky_xy,husky_orientation
    husky_xy[2][0] = data3.pose.pose.position.x
    husky_xy[2][0] = data3.pose.pose.position.y
    husky_orientation[2][0] = data3.pose.pose.orientation.z

def position_husky4(data4):
    #global husky4_y, husky4_x
    global husky_xy,husky_orientation
    husky_xy[3][0] = data4.pose.pose.position.x
    husky_xy[3][1] = data4.pose.pose.position.y    
    husky_orientation[3][0] = data4.pose.pose.orientation.z
...
def main():
    ...
    pub_vel_husky4 = rospy.Publisher('/Husky4/husky_velocity_controller/cmd_vel',Twist, queue_size=10)
    
    #SUBSCRIBERS for HUSKIES:
    ...
    rospy.Subscriber('/Husky3/husky_velocity_controller/odom', Odometry, position_husky3)
    rospy.Subscriber('/Husky4/husky_velocity_controller/odom', Odometry, position_husky4)
...
#('dest: ',)
   ...
   142.]
    ...
    for mc1 in range(MC):            
        ...
        Pc = ra
        threshold = 10.0
        dps=0.25 # speed of agents, reducing by 4x
        ...
        #Sh = numpy.asarray([S1,S3]).reshape(nos,2)
        ...
        while not rospy.is_shutdown():
            ...
            #H1.linear.x = ds #checing where the bot moves in rviz as compared to gazebo
            #H1.angular.z = 0.0
            pub_vel_husky1.publish(H1)
            pub_vel_husky2.publish(H2)
            pub_vel_husky3.publish(H3)
            pub_vel_husky4.publish(H4)
            ...
            if(toggle == 0):
                ...
                if(out1 == 0 or out2 == 0):
                    ...
                    if(len(g1) >= 0.1*(numpy.size(grp1)) and eating1 == 0):
                        ...
                        S0,mode1,I1,direction1 = sheperdMovement(numpy.asarray([Sh[0],Sh[2]]).reshape(2,2),initpoints[0][g1],initpoints[1][g1],ra,Pd1,Pc,destination1,f1,mindisShep,rhoash,alpha,ang,mode1,nin,ds,rs,method1,predmasterslave1,decrad,q,I1)
                    else:
                    ...
                    if(len(g2) >= 0.1*(numpy.size(grp2)) and eating2 == 0):
                        ...
                        Sh1,mode2,I2,direction2 = sheperdMovement(numpy.asarray([Sh[1],Sh[3]]).reshape(2,2),initpoints[0][g2],initpoints[1][g2],ra,Pd2,Pc,destination2,f2,mindisShep,rhoash,alpha,ang,mode2,nin,ds,rs,method2,predmasterslave2,decrad,q,I2)
                    else:
                        #out2 = 1;
                        eating2 = 1;
                    ...
                    #odom also gives orientation, so we've the current orientation\
                    #we now have the new orientation, we need to publish the velocity and then take the new position
                    #of the huskies and then put this new pos in the Sh
                    change_in_theta_1 = numpy.arctan2(direction1[0][1],direction1[0][0]) - husky_orientation[0][0]
                    change_in_theta_2 = numpy.arctan2(direction1[1][1],direction1[1][0]) - husky_orientation[1][0]
                    change_in_theta_3 = numpy.arctan2(direction1[0][1],direction1[0][0]) - husky_orientation[2][0]
                    change_in_theta_4 = numpy.arctan2(direction1[1][1],direction1[1][0]) - husky_orientation[3][0]

                    H1.linear.x = ds
                    H1.angular.z = change_in_theta_1

                    H2.linear.x = ds
                    H2.angular.z = change_in_theta_2

                    H3.linear.x = ds
                    H3.angular.z = change_in_theta_3

                    H4.linear.x = ds
                    H4.angular.z = change_in_theta_4
                    print("change in theta:")
                    print(change_in_theta_1 , " " , change_in_theta_2 , " " , change_in_theta_3)
                    Sh[0][0] = S0[0][0]
                    Sh[0][1] = S0[0][1]
                    Sh[2][0] = S0[1][0]
                    Sh[2][1] = S0[1][1]                
                ...
                if(out1 == 1 and out2 == 1 and (in1 == 0 or in2 == 0)): #and pos_update1 == 1 and pos_update2 == 1
                    ...
                    if(len(g1) > ceil(0.1*len(grp1)) and in1 == 0 and too_close == 0):
                        ...
                        S0,mode1,I1,direction1 = sheperdMovement(numpy.asarray([Sh[0],Sh[2]]).reshape(2,2),initpoints[0][g1],initpoints[1][g1],ra,Pd1,Pc,destination,f1,mindisShep,rhoash,alpha,ang,mode1,nin,ds,rs,method1,predmasterslave1,decrad,q,I1)      
                    ...
                    if(len(g2) > ceil(0.1*len(grp2)) and in2 == 0 and too_close == 0):#if too_close becomes 1, then in2 will become 1
                        ...
                        Sh1,mode2,I2,direction2 = sheperdMovement(numpy.asarray([Sh[1],Sh[3]]).reshape(2,2),initpoints[0][g2],initpoints[1][g2],ra,Pd2,Pc,destination,f2,mindisShep,rhoash,alpha,ang,mode2,nin,ds,rs,method2,predmasterslave2,decrad,q,I2)                      
                    ...
                    H1.linear.x = ds
                    H1.angular.z = 2*change_in_theta_1

                    H2.linear.x = ds
                    H2.angular.z = 2*change_in_theta_2

                    H3.linear.x = ds
                    H3.angular.z = 2*change_in_theta_3

                    H4.linear.x = ds
                    H4.angular.z = 2*change_in_theta_4                    
                    # if(too_close == 0):
            ...
            for i in range(nos):    
                ...
                if(toggle == 0):#iniitially we need to move the huskies towards the swarm
                    ...
                    M.markers[N1+i].pose.position.x = husky_xy[i][0]
                    M.markers[N1+i].pose.position.y = husky_xy[i][1]
                if(toggle == 1):#From gazebo
                    M.markers[N1+i].pose.position.x = Sh[i][0]
