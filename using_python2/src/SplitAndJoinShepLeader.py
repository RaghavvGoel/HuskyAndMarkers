...
def main():
    ...
    for i1 in range(N+nos+nod): #to include shepherd too
        ...
        if(i1 >= N+nos):
            ...
            if(l == 0):
                ...
                marker.pose.position.x = 10000
                marker.pose.position.y = 10000
    ...
    while not rospy.is_shutdown():
        ...
        if(toggle == 0):
            ...
            if(len(g1) > 0.1*(numpy.size(grp1)) and eating1 == 0):
                ...
                if(orient1 == 0 and leader_front1 == 0): #+++++++++++++++++++++++++++++++++++++++++++++++++++++++
                    ...
                    sub_dest1[0] = numpy.average(initpoints[0,g1]) + 5.0*numpy.cos(m1)
                    sub_dest1[1] = numpy.average(initpoints[1,g1]) + 5.0*numpy.sin(m1)
                ...
                #sheepmovements(initpoints,((len(g1))),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode1,destination1,p,swarm_split)
                eating1 = 1;
                m1 = numpy.arctan2((destination[1] - destination2[1]),(destination[0] - destination1[0]))
                if(m1 > pi/2):
                    alpha1 = pi - m1;
                else:
                    alpha1 = m1;
            ...
            if(len(g2) > 0.1*(numpy.size(grp2)) and eating2 == 0):
                ...
                if(orient2 == 0): #+++++++++++++++++++++++++++++++++++++++++++++++++++++++
                    ...
                    sub_dest2[0] = numpy.average(initpoints[0,g2]) + 5.0*numpy.cos(m2)
                    sub_dest2[1] = numpy.average(initpoints[1,g2]) + 5.0*numpy.sin(m2)
        ...
        if(eating1 == 1 and out1 == 0):
            M.markers[N1+nos].scale.x = M.markers[N1+nos].scale.x - 0.25
            M.markers[N1+nos].scale.y = M.markers[N1+nos].scale.y - 0.25
        ...
        if(eating1 == 1 and pos_update1 == 0):    
            ...
            if not (Sh[0][0] >= dest1[0] and Sh[0][1] <= dest1[1]):
                ...
                if(numpy.where(D1 < rs)):
                    S0, a = moveSheperd(S0,numpy.asarray(destination1).reshape(1,2),1,-1)
        ...
        if(eating2 == 1 and out2 == 0):
            M.markers[N1+nos+1].scale.x = M.markers[N1+nos+1].scale.x - 0.25
            M.markers[N1+nos+1].scale.y = M.markers[N1+nos+1].scale.y - 0.25
        ...
        if(eating2 == 1 and pos_update2 == 0):        
            ...
            if not (Sh[1][0] <= dest2[0] and Sh[1][1] <= dest2[1]):
                ...
                if(numpy.where(D2 < rs)):
                    S1, a = moveSheperd(S1,numpy.asarray(destination2).reshape(1,2),1,-1)
        ...
        if(leader_front2 == 0 and orient1 == 1):
            ...
            if(Sh[1][0] > GCM2[0] and Sh[1][1] < GCM2[1]):
                leader_front2 = 1;
