...
def main():
    rospy.init_node('PhysicometicsLeader', anonymous=True)

    pub_marker = rospy.Publisher('Physico_leader', MarkerArray , queue_size=1)
    ...
    for mc1 in range(MC):
        ...
        while not rospy.is_shutdown():
            ...
            if(swarm_split == 1):
                DD1 = numpy.linalg.norm(initpoints.T - initpoints[:,leaderidx[0]].T,axis = 1)
                DD2 = numpy.linalg.norm(initpoints.T - initpoints[:,leaderidx[1]].T,axis = 1)
                far_from1 = numpy.where(DD1 > 3*rs)
                far_from2 = numpy.where(DD2 > 3*rs)
                separated_agents = numpy.intersect1d(far_from1[0],far_from2[0])
                new_grp1 = numpy.setdiff1d(grp1,separated_agents)
                new_grp2 = numpy.setdiff1d(grp1,separated_agents)
                grp1 = new_grp1
                grp2 = new_grp2                
                agents_sep[0][mc1] = numpy.size(separated_agents)
            ...
            if(eating3 == 1):
                M.markers[N+nos+2].scale.x = M.markers[N+nos+2].scale.x - size_dec         
                M.markers[N+nos+2].scale.y = M.markers[N+nos+2].scale.y - size_dec
        ...
        #net.append(stop_time2 - start_time2)
    ...
    print("separated_agents: " , agents_sep)
