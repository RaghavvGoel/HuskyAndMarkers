...
def SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,leader_pos,leaderRrep,leaderRori,leaderRatt,destination,destinationthreshold,nol,leader_vels,destination_other,nos):
    pos= initpoints.T
    #print("pos:")
    #print(pos)
    # if(nol == 2):
        ...
        if(flag != -1):
            ...
            for j in range(numpy.size(v)):
                ...
                if(v[j] < N and i < N and v[j] not in ind):
                    norm1 = numpy.linalg.norm(vels[i])
                    norm2 = numpy.linalg.norm(vels[v[j]])
                    #CHANGED FROM DENO TO NORM1*NORM2
                    deno = numpy.sqrt(pow(vels[v[j]][1],2)+pow(vels[v[j]][0],2))*numpy.sqrt(pow(vels[i][1],2)+pow(vels[i][0],2))
                    if(abs(numpy.arccos((vels[v[j]][1]*vels[i][1] + vels[v[j]][0]*vels[i][0])/(norm1*norm2))) <= phi):
            ...
            if(numpy.size(leader_pos) != 0 and i < N):
                dpred = [0, 0]
                #distp = []
                for j in range(nol):
                    #print("EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEe")
                    #print(leader_pos)
                    #print("j= " , j , " sizeOfS= " , numpy.size(leader_pos) , " " , S , " " , S[0])
                    #distp.append(numpy.sqrt(pow(pos[i,0]-S[j][0],2) + pow(pos[i,1]-S[j][1],2)))
                    distp = numpy.linalg.norm(pos[i] - leader_pos[j] , axis = 0)
                    #print("distp= " , distp , " pos[i]= " , pos[i], " S= " , S)
                ...
                for j in range(numpy.size(v)):
                    ...
                    dpred = dpred + (leader_pos[v[0][j],:]-pos[i,:])/numpy.linalg.norm((leader_pos[v[0][j],:]-pos[i,:]),axis = 0)
                    dpredFlag = 1
            ...
            if(i in leader_idx and numpy.size(leader_pos) != 0):
                ...
                #GCM = numpy.asarray(GCM).reshape(1,2)
                D1 = numpy.linalg.norm(initpoints.T - leader_pos[0],axis=1)
                D2 = numpy.linalg.norm(initpoints.T - leader_pos[1],axis=1)
                grp1 = numpy.where(D1 < D2)
                grp2 = numpy.where(D1 >= D2)
                ...
                gcm1 = numpy.asarray(gcm1).reshape(1,2)
                gcm2 = numpy.asarray(gcm2).reshape(1,2)
                D_gcm1 = numpy.linalg.norm(pos[grp1[0]] - gcm1, axis=1)
                D_gcm2 = numpy.linalg.norm(pos[grp2[0]] - gcm2, axis=1) 
                #D_gcm = numpy.linalg.norm(pos - GCM, axis=1)
                thresh = 10.0
                idx_D1 = numpy.where(D_gcm1 >= thresh)
                idx_D2 = numpy.where(D_gcm2 >= thresh)            
                #idx_D_gcm = numpy.where(D_gcm >= rs/2.0) #Agent with distance greater than 8 from GCM are being taken as ownrdered off
                new_pos = numpy.zeros((1,2))    
                if(i == N):
                #if(i in leader_idx):
                    #dleader1 = dleader1 + (destination[0,:]-leader_pos[i-N,:])/numpy.linalg.norm((destination[0,:]-leader_pos[i-N,:]),axis = 0)
                    #print("dleader1 initially: " , dleader1)
                    if(numpy.size(idx_D1) == 0):
                        slope = numpy.arctan2(gcm1[0][1]-destination[0][1],gcm1[0][0]-destination[0][0])
                        new_pos[0][0] = gcm1[0][0] + 3*numpy.cos(slope)
                        new_pos[0][1] = gcm1[0][1] + 3*numpy.sin(slope)
                        print("new_pos: " , new_pos , " GCM1: " , gcm1)
                        print("slope herding: " , slope)
                        #if(numpy.linalg.norm(leader_pos[i-N] - GCM[0], axis = 0) > rs):
                        dleader1 = dleader1 + (new_pos[0,:]-leader_pos[i-N,:])/numpy.linalg.norm((new_pos[0,:]-leader_pos[i-N,:]),axis = 0)        
                        # else:
                        #     #dleader1 = dleader1 + (destination[0,:]-leader_pos[i-N,:])/numpy.linalg.norm((destination[0,:]-leader_pos[i-N,:]),axis = 0)            
                        #     dleader1 = dleader1 + (GCM[0,:]-leader_pos[i-N,:])/numpy.linalg.norm((GCM[0,:]-leader_pos[i-N,:]),axis = 0)        
                    else:
                        max_D = numpy.where(numpy.max(D_gcm1) == D_gcm1)
                        print("max_D= " , max_D , " ")
                        slope = numpy.arctan2((pos[max_D[0],1] - gcm1[0][1]),(pos[max_D[0],0] - gcm1[0][0]))
                        print("slope: " , slope , " pos: " , pos[max_D[0]] , " GCM: " , gcm1)
                        new_pos[0][0] = pos[max_D[0],0] + 3.0*numpy.cos(slope)
                        new_pos[0][1] = pos[max_D[0],1] + 3.0*numpy.sin(slope)
                        print("new_pos(collectng)= " , new_pos)
                        dleader1 = dleader1 + (new_pos[0,:]-leader_pos[i-N,:])/numpy.linalg.norm((new_pos[0,:]-leader_pos[i-N,:]),axis = 0)        
                    #dleader1 = dleader1/2.0
                    dleaderFlag1 = 1
                    # print("LLLLLLLLLLLLLLLLLLLLLLLLLLLLLL")
                    # print("dleader1 after adding GCM: " , dleader1 , " GCM= " , GCM)
                    # print("")
                    #print("norm1: " , numpy.linalg.norm(dleader1,axis = 0))
                elif(i == N+1):
                    if(numpy.size(idx_D1) == 0):
                        slope = numpy.arctan2(gcm2[0][1]-destination_other[0][1],gcm2[0][0]-destination_other[0][0])
                        new_pos[0][0] = gcm2[0][0] + 3*numpy.cos(slope)
                        new_pos[0][1] = gcm2[0][1] + 3*numpy.sin(slope)
                        print("new_pos: " , new_pos , " GCM: " , gcm2)
                        print("slope herding: " , slope)
                        #if(numpy.linalg.norm(leader_pos[i-N] - GCM[0], axis = 0) > rs):
                        dleader2 = dleader2 + (new_pos[0,:]-leader_pos[i-N,:])/numpy.linalg.norm((new_pos[0,:]-leader_pos[i-N,:]),axis = 0)        
                        # else:
                        #     #dleader1 = dleader1 + (destination[0,:]-leader_pos[i-N,:])/numpy.linalg.norm((destination[0,:]-leader_pos[i-N,:]),axis = 0)            
                        #     dleader1 = dleader1 + (GCM[0,:]-leader_pos[i-N,:])/numpy.linalg.norm((GCM[0,:]-leader_pos[i-N,:]),axis = 0)        
                    else:
                        max_D = numpy.where(numpy.max(D_gcm2) == D_gcm2)
                        print("max_D= " , max_D , " ")
                        slope = numpy.arctan2((pos[max_D[0],1] - gcm2[0][1]),(pos[max_D[0],0] - gcm2[0][0]))
                        print("slope: " , slope , " pos: " , pos[max_D[0]] , " GCM: " , gcm2)
                        new_pos[0][0] = pos[max_D[0],0] + 3.0*numpy.cos(slope)
                        new_pos[0][1] = pos[max_D[0],1] + 3.0*numpy.sin(slope)
                        print("new_pos(collectng)= " , new_pos)
                        dleader2 = dleader2 + (new_pos[0,:]-leader_pos[i-N,:])/numpy.linalg.norm((new_pos[0,:]-leader_pos[i-N,:]),axis = 0)        
                    #dleader1 = dleader1/2.0
                    dleaderFlag2 = 1                #     dleader2 = dleader2 + (destination_other[0,:]-leader_pos[i-N,:])/numpy.linalg.norm((destination_other[0,:]-leader_pos[i-N,:]),axis = 0)        
                #     dleaderFlag2 = 1
                    #print("LLLLLLLLLLLLLLLLLLLLLLLLLLLLLL")
                ...
                # %% decision making
            di = [0, 0]    
            if(dpredFlag == 2): #dpredflag made = 2, so that never true as we want to check the below changes made
                #di = -dpred
                di[0] = di[0] - dpred[0]
                di[1] = di[1] - dpred[1]
                nagentFlag = 1;
            ...
            if(nagentFlag > 0):
                ...
                if(i < N):
                    ...
                    leader_pos[i-N][0] = leader_pos[i-N][0]+numpy.cos(dangle1)*s*dt*(sp) # we'll change the speed of the leader
                    leader_pos[i-N][1] = leader_pos[i-N][1]+numpy.sin(dangle1)*s*dt*(sp)                    
            else:
                if(i < N):
                    tempdi[i] = vels[i]
                    ...
                    # leader_vels = vels[i]
                    leader_pos[i-N][0] = leader_pos[i-N][0] + leader_vels[i-N][0]*s*dt*(sp)
                    leader_pos[i-N][1] = leader_pos[i-N][1] + leader_vels[i-N][1]*s*dt*(sp) 
                    # temppos[i][1] = pos[i,1] + vels[i,1]*s*dt                    
...
def main():
    ...
    nol = 2 #testing shepehrd so making this ZERO
    leader_pos1 = [0.0,-45.0]
    leader_pos2 = [0.0, 5.0]
    ...
    rs = 12.5;
    nos = 0;
    #S = numpy.asarray(S).reshape(nos,2)
    ShepVels = numpy.zeros((1,2))
    ShepVels[0][0] = numpy.random.rand(1,1)
    ShepVels[0][1] = numpy.random.rand(1,1)

    M = MarkerArray()    
    l = 0;
    shape = Marker.CUBE;
    for i in range(N+ nol + nos +2): #to include shepherd too
        marker = Marker()
        #print("i " , i)
        ...
        elif(i >= N + nol and i < N + nol + nos): 
            marker.ns = "shep"
            marker.pose.position.x = S[i - (N+nol)][0];
            marker.pose.position.y = S[i - (N+nol)][1];
            marker.color.r = 1.0;
            marker.color.g = 1.0;               
    ...
    near_swarm1 = 0; near_swarm2 = 0; near_swarmS1 = 0;
    in1 = 0; in2 = 0;
    eaten1 = 0; eaten2 = 0;
    ...
    while not rospy.is_shutdown():
        ...
        #if(near_swarm1 == 1 and near_swarm2 == 1 and (eaten1 == 0 or eaten2 == 0)):
        if(near_swarm1 == 1 and near_swarm2 == 1):
            initpoints,vels,LeaderPos,LeaderVels =  SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,LeaderPos,leaderRrep,leaderRori,leaderRatt,destination1,destinationthreshold,nol,LeaderVels,destination2,nos)        
            if(numpy.linalg.norm((LeaderPos[0] - destination1[0]),axis = 0) <= destinationthreshold  and in1 == 1 and entered1 == 0):
                ...
                # if(m1 > pi/2):
                #     m1 = m1 - pi/2
                dest1[0][0] = destination1[0][0] + (15.0)*numpy.cos(m1);
            ...
            #     # if(m2 > pi/2):
            #     #     m2 = m2 - pi/2
            #     dest2[0][0] = destination2[0][0] + (15.0)*numpy.cos(m2);
            #     dest2[0][1] = destination1[0][1] + 15*numpy.sin(m2);    
        ...
        elif(near_swarm1 != 1 and near_swarm1 != 1):#Rori same as Rrep => swarm state
            initpoints,vels,notused,notused =  SheepMovement(initpoints,vels,Rrep,Rrep,Ratt,1.5*s1,dt,phi,omega,S,rs,[],leaderRrep,leaderRori,leaderRatt,numpy.asarray([0,0]).reshape(1,2),0,0,[],numpy.asarray([0,0]).reshape(1,2),nos)        

        
        elif(eaten1 == 1 and eaten2 == 1):
                initpoints,vels,LeaderPos,LeaderVels =  SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,LeaderPos,leaderRrep,leaderRori,leaderRatt,destination,destinationthreshold/3,nol,LeaderVels,destination,nos)                        
        ...
        for i in range(N + nol + nos):
            ...
            #     #print("LLLLLLLLLLLLLLLLLLLLLLLLL")
            #     #print(leader_pos1)
            #     M.markers[i].pose.position.x = S[0][0]
            #     M.markers[i].pose.position.y = S[0][1]            
        ...
        #     sp1 = numpy.linalg.norm(LeaderPos[0] - GCM)*(0.1)                                                           
        #     #print("sp1= " , sp1)         
        #     idxD1 = numpy.where(D1 <= rs/1.5)            
        #     if(numpy.size(idxD1) > 0):
        #         near_swarm1 = 1;
        #     else:
        #         LeaderPos[0],a = moveLeader(LeaderPos[0],GCM,sp1*dt,1)

        # if(near_swarm2 == 0 and nol >=2):
        #     D2 = numpy.linalg.norm(initpoints.T - LeaderPos[1],axis=1)
        #     sp2 = numpy.linalg.norm(LeaderPos[1] - GCM)*(0.1)
        #     #print(" sp2= " , sp2)
        #     idxD2 = numpy.where(D2 <= rs/1.5)            
        ...
        if(near_swarm1 == 0 or near_swarm2 == 0):
            if(LeaderPos[0][1] < LeaderPos[1][1]):
                sp1 = numpy.linalg.norm(LeaderPos[0] - GCM)*(0.1)                                                           
                sp2 = numpy.linalg.norm(LeaderPos[1] - GCM)*(0.1)                                 
                LeaderPos[0],a = moveLeader(LeaderPos[0],GCM,0.5,1)
                LeaderPos[0],a = moveLeader(LeaderPos[1],GCM,0.5,1)
            ...
            # if(near_swarmS1 == 0):
            #     #print("in if................")            
            #     DS1 = numpy.linalg.norm(initpoints.T - S[0],axis=1)
            #     spS1 = numpy.linalg.norm((S[0] - GCM),axis=0)*(0.1)
            #     #print(" sp2= " , sp2)
            #     idxDS1 = numpy.where(DS1 < rs)        
            #     #print(idxDS1)   
            #     #print(S) 
            #     if(numpy.size(idxDS1) > 0):
            #         near_swarmS1 = 1
            #     else:
            #         S,a = moveLeader(S,GCM,spS1*dt,1)                        

            #print("near_swarmS1= " , near_swarmS1 , "idxDS1= " , idxDS1)        
        ...
        speed = 0.3;            
        if(entered1 == 1 and (LeaderPos[0][0] < dest1[0][0] or LeaderPos[0][1] < dest1[0][1]) and eaten1 == 0):
            dest1[0][1] = destination1[0][1] + 15*numpy.sin(m1);

            LeaderPos[0],a = moveLeader(LeaderPos[0],dest1,speed,1) 
            D_from_L1 = numpy.linalg.norm(initpoints.T - LeaderPos[0],axis =1)      
            close_to_L1 = numpy.where(D_from_L1 < leaderRrep)
        ...
        if(entered2 == 1 and (LeaderPos[1][0] > dest2[0][0] or LeaderPos[1][1] < dest2[0][1]) and eaten2 == 0):
            ...
            close_to_L2 = numpy.where(D_from_L2 < leaderRrep)
