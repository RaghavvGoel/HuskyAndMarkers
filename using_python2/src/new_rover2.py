...
def get_position_local(msg):    
    global local_lat2 , local_lon2 , flag31 , ini_local_lat2 , ini_local_lon2;
...
def get_compass(msg):
    global heading_angle2 , flag41 , ini_angle1
    heading_angle2 = msg.data    
    if(flag41 == 0):
        ini_angle1 = heading_angle2
        #ini_angle1 = 90
        flag41 = 1
...
def main():
    rospy.init_node('rover_sheperd', anonymous=True)

    pub_marker = rospy.Publisher('rover_sheperd', MarkerArray , queue_size=1)
    ...
    #Xstartpos=0.0 #swarm initial position
    #Ystartpos=4.0
    ...
    # D.header.frame_id = "/my_marker_sheep";
    ...
    # while(heading_angle < 85 or heading_angle > 95):            
    #     rov.twist.linear.x = 0
    ...
    while not rospy.is_shutdown():    
        ...
        if(N > 0):
            #print(initpoints[0][outthreshold])
            #print("x_out " , x_out)
            #print("y_out " , y_out)
            print("yoyo")
            Sh,mode,I = sheperdMovement(Sh,x_out,y_out,ra,Pd,Pc,destination,f,mindisShep,rhoash,alpha,ang,mode,nin,ds,rs,method,predmasterslave,decrad,q,I)
        else:
            continue        
        # LIMIT ANGLE HERE
        ...
        for i in range(nos):    
            M.markers[i].pose.position.x = temp_sh[i][0];
            M.markers[i].pose.position.y = temp_sh[i][1];
        ...
        if(N > 0):
            ...
            #print("X_out: " , x_out)
            for i in range(N):
                dist.append(sqrt(pow(X[i] - destination[0],2) + pow(Y[i] - destination[1],2)))
                if(sqrt(pow(X[i] - destination[0],2) + pow(Y[i] - destination[1],2)) <= threshold):
                    flag = flag + 1
            ...
            for i in outthreshold:        
                x_out.append(X[i])
                y_out.append(Y[i])        

            print("OUTTHRESHOLD: " , len(outthreshold))    
        #print("Deleting " , i  ," " )
        #print(dist)
        ...
        dx2 = (Sh[1,0] - temp_sh[1,0])
        dy2 = (Sh[1,1] - temp_sh[1,1])
        ...
        #angle_to_publish = thetadeg - (heading_angle - ini_angle)                
        #angle_to_publish = thetadeg - angle_covered 
        #angle_to_publish = angle_to_publish%360
        ...
        while(heading_angle2 < beta2 - 10 or heading_angle2 > beta2 + 10):
            ...
            if(heading_angle2 <= beta2):
                ...
                if(heading_angle - beta <= 180):
                    yaw_to_publish = 1.0
                else:
                    yaw_to_publish = -1.0                    

            
            rov2.twist.angular.z = speed_of_angular_change         
            rvel2.publish(rov2)
        ...
        while(rospy.get_rostime() < time1 + rospy.Duration(0.8)):
            print("in time loop")

        #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if(flag >= numpy.ceil(0.9*N1)):
