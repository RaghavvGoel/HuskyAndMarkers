...
def delay(dur):
    t = rospy.get_rostime()
    while(rospy.get_rostime() < t + rospy.Duration(dur)):
        #print("in delay")
        countx = 1
...
def main():
    ...
    for i in range(N+nos+1): #to include shepherd too
        ...
        if(i >= N and i<N+nos):
            ...
            marker.pose.orientation.z = 22.0/7;
            marker.scale.x = 1.0;
            marker.scale.y = 0.1;
            marker.color.r = 1.0;
            marker.color.g = 0.0;
    ...
    while not rospy.is_shutdown():
        ...
        for i in range(nos):
            theta = numpy.arctan2(Sh[0][1], Sh[0][0]);
            alpha = numpy.arctan2(temp_sh[0][1] , temp_sh[0][0]);
            #M.markers[N1+i].pose.orientation.z = numpy.arctan2(temp_sh[0][1] , temp_sh[0][0])
            #tempz = numpy.arctan2(temp_sh[0][1] , temp_sh[0][0])
            while(alpha != theta):
                M.markers[N1+i].pose.orientation.z = alpha + sign/(2*pi);
                alpha = alpha + sign/(2*pi);
                print("need angle: ", theta ,  " current: " , alpha )
                pub_marker.publish(M)
                if((alpha > 25 and sign == 1)or (alpha < -25 and sign == -1)):
                    sign = sign*(-1);
                pub_marker.publish(M)     
                delay(0.5)
            #M.markers[N1+i].pose.orientation.z = numpy.arctan2(Sh[0][1]-temp_sh[0][1], Sh[0][0] - temp_sh[0][0]);
            #M.markers[N1+i].pose.position.x = Sh[i][0];
            #M.markers[N1+i].pose.position.y = Sh[i][1];
            M.markers[N1+i].pose.position.x = ds*cos(theta)
            M.markers[N1+i].pose.position.y = ds*sin(theta)
