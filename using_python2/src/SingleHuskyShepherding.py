...
huskyX = [0.0 , 0.0, 0.0, 0.0]
huskyY = [0.0 , 0.0, 0.0, 0.0]
Yaw = [0.0 , 0.0, 0.0, 0.0]
...
def sheepmovements(initpoints,n,c,rs,Shepherd,ra,rhoa,rhos,unitvec,h,e,dps,mode,destination,p,swarm_split):
    ...
    if(len(idx) != 0):
        ...
        idx = numpy.setdiff1d(list(xrange(len(initpoints[0]))),idx)
    #print("idx: " , idx)
    #print("idx1: " , idx1)
        if(len(idx) != 0):
...
def rviz_to_gazebo(theta):
    #angle_gazebo = theta - (-pi/2); # -pi/2 as this is the initial angle wrt rviz
    angle_gazebo = theta - 0.0 #
    angle_gazebo = angle_gazebo%(2*pi)
    if(angle_gazebo > pi):
        angle_gazebo = angle_gazebo - 2*pi; #making the output between -pi to pi
    return angle_gazebo

def gazebo_to_rviz(x,y,theta,initialX,initialY):
    #x = initialX + x
    #y = initialY + y 
    new_rvizX = x*numpy.cos(theta) - y*numpy.sin(theta)
    new_rvizY = x*numpy.sin(theta) + y*numpy.cos(theta)
    new_rvizX = initialX + new_rvizX
    new_rvizY = initialY + new_rvizY
    return [new_rvizX, new_rvizY] 
...
def main():
    ...
    destination1x = [0. , -71. ,-108. ,-140. , -63. , -82. , -78. ,-116. , -63. , -96., -119. , -71.,  -109. , -62. ,-105. , -91. ,-142. , -68. ,-128. ,-133. ,-136. , -80. ,-128. , -59.,
  -130.]
    destination1y =  [  40.0,  108. ,-124.,  -24. , 227. ,  53., -185.,   78.,   90.,   55., -192. ,  68.,
...
#('dest: ',)
   ...
   142.]
    ...
    pub_marker = rospy.Publisher('multi_sheep_sheperd_no200', MarkerArray , queue_size=1)
    ...
    #Shepherd=[-0,100]
        ...
        Xstartpos=40 #swarm initial position
        Ystartpos=0
        h = 0.5 #strength of inertial force
        N = 3 # number of agents
        N1 = copy.deepcopy(N)
        ra=3.0  # distance for self repulsion, changing from 2.0 to 3.0 as we aren't playing with point masses
        mindisShep = 6
        #n=numpy.ceil(0.4*N1)
        n = N*90/100
        Pd = ra*log(N)
        ...
        dps = 1 # speed of agents dividing by 2.0, as shepherd speed is reduced to 0.5
        ...
        rs = 15.0  #radius of influence
        c=1.05 #strength of clustering
        nod = 3 #no. of destination
        threshold = 10.0
        #______________________________________________________________________________________________
        #Check which destination is near and then assign accordingly
        
        # mc_d = numpy.zeros((1,2))
        # mc_dist = numpy.zeros((2,3))
        # mc_dist[0][0] = destination1x[mc]
        # mc_dist[1][0] = destination1y[mc]
        # mc_dist[0][1] = destination2x[mc]
        # mc_dist[1][1] = destination2y[mc]
        # mc_dist[0][2] = destinationx[mc]
        # mc_dist[1][2] = destinationy[mc] 
        destination = [destinationx[mc],destinationy[mc]]
        destination1 = [destination1x[mc],destination1y[mc]]
        destination2 = [destination2x[mc],destination2y[mc]]


        #mc_d[0][2] = numpy.sqrt(pow(mc_destx[mc] - Xstartpos,2) + pow(mc_desty[mc] - Ystartpos,2))
        # mc_d[0][0] = numpy.sqrt(pow(mc_dest1x[mc] - Xstartpos,2) + pow(mc_dest1y[mc] - Ystartpos,2))
        # mc_d[0][1] = numpy.sqrt(pow(mc_dest2x[mc] - Xstartpos,2) + pow(mc_dest2y[mc] - Ystartpos,2))

        # mc_sort = numpy.argsort(mc_d)

        # destination=[mc_destx[mc],mc_desty[mc]]
        # destination1 = [mc_dist[0][mc_sort[0][0]],mc_dist[1][mc_sort[0][0]]]
        # destination2 = [mc_dist[0][mc_sort[0][1]],mc_dist[1][mc_sort[0][1]]]
        #______________________________________________________________________________________________
        ...
        swarm_split = 0;
        eating1 = 0; eating2 = 0;
        eaten1 = 0; eaten2 = 2; eaten3 = 2; #Declared 2 so that dont enter if condition.
        sub_dest1 = [0.0,0.0]
        num_separated_terms = 0
        initial_angle = 0.0; #angle of the bot in gazebo
        initial_gazeboX = 0.0; initial_gazeboY = 0.0;
        k = 0.3 #with k=1.0, the huskies reached perpendicular goal
        speed_sheep = numpy.zeros(N);
        delta_theta = numpy.zeros(N)
        change_in_heading = numpy.zeros(N)
        dist_left_to_cover = numpy.zeros(N)

        start_time[0][mc] = rospy.get_rostime().to_sec()
        while not rospy.is_shutdown():
            ...
            pub_vel_husky3.publish(H3)
            pub_vel_husky0.publish(H0)
            ...
            print("sheep before:")
            print(initpoints)
            if(eating1 == 1 or eating2 == 1):
                sheepmovements(initpoints,n,c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode,[100000.0,0.0],p,swarm_split)
                if(enter_once == 0):
                    m = numpy.arctan2(destination1[1]-destination2[1],destination1[0]-destination2[0])
                    sub_dest1[0] = destination1[0] + 7.5*numpy.cos(m)
                    sub_dest1[1] = destination1[1] + 7.5*numpy.sin(m)
                    enter_once = 1;
                    #mode = 2;
            #     #print("LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL")    
            #     #print("sub_dest1: " , sub_dest1, " destination1 " , destination1)    
            else:
                enter_once = 0;
                sheepmovements(initpoints,n,c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode,destination1,p,swarm_split)
                Sh,mode,I,direction = sheperdMovement(Sh,initpoints[0][outthreshold],initpoints[1][outthreshold],ra,Pd,Pc,destination1,f,mindisShep,rhoash,alpha,ang,mode,nin,ds,rs,method,predmasterslave,decrad,q,I)                
                #print("direction in rviz: " , numpy.arctan2(direction[0][1],direction[0][0])*180/pi)
                #angle_from_new_Sh = numpy.arctan2(new_Sh[0][1] - Sh[0][1], new_Sh[0][0] - Sh[0][0])
                #print("angle from position of shepherd:" , angle_from_new_Sh*180/pi)
                #print("tan: " , numpy.arctan2(direction[0][1],direction[0][0]))
                #new_Sh = Sh

                #new_yaw_gazebo = numpy.arctan2(direction[0][1],direction[0][0])
                #converted_new_yaw_gazebo = rviz_to_gazebo(new_yaw_gazebo) #converts to gazebo coordinate frame
                #converted_new_yaw_gazebo = new_yaw_gazebo
                #print("direction to be taken in gazebo: " , converted_new_yaw_gazebo*180/pi)
                #print("old yaw: " , Yaw[0]*180/pi)
            # else:
            ...
            for i in range(N):
                ...
                #change_in_heading[i] = numpy.arctan2(initpoints[1][i] - temp_initpoints[1][i], initpoints[0][i] - temp_initpoints[0][i])
                change_in_heading[i] = numpy.arctan2(unitvec[1][i] , unitvec[0][i])
                # print("i= " , i , "   ...............................")
                # print("deltaY: " , initpoints[1][i] - temp_initpoints[1][i] , " " , initpoints[1][i] , " " , temp_initpoints[1][i])
                # print("deltaX: " , initpoints[0][i] - temp_initpoints[0][i] , " " , initpoints[0][i] , " " , temp_initpoints[0][i])
                # print("change_in_heading: "  , change_in_heading[i])
                
                delta_theta[i] = change_in_heading[i] - Yaw[i]; #we can scale this
                #print("delta_theta: " , delta_theta[i])
                temp_initpoints = copy.deepcopy(initpoints)
                ...
                #print("postion: " , huskyX[i] , " " , huskyY[i])
                #print("temp init: " , temp_initpoints[:,i])
                dist_left_to_cover[i] = numpy.sqrt(pow(temp_initpoints[1][i]-huskyY[i],2) + pow(temp_initpoints[0][i]-huskyX[i],2))
                if(dist_left_to_cover[i] < 0.01 or i in inthreshold):
                    speed_sheep[i] = 0.0;
                else:
                    speed_sheep[i] = 0.5*dist_left_to_cover[i] #?? how will this work = works amazingly good
                #print("dist_left_to_cover: " , dist_left_to_cover[i])
                #print("speed_sheep: " , speed_sheep[i])    
            ...
            for i in range(nos):    
                ...
                # if(toggle == 1):
                ref_angle = 0.0
                #new_rviz_coordinates = gazebo_to_rviz(huskyX[i],huskyY[i],ref_angle,initialX,initialY)
                M.markers[N1+i].pose.position.x = huskyX[N1+i] #new_rviz_coordinates[0]  #Sh[i][0]
                M.markers[N1+i].pose.position.y = huskyY[N1+i] #new_rviz_coordinates[1]  #Sh[i][1]
                
                #print("rviz_pos:")
                #print(new_rviz_coordinates)
                #print("gazebo pos")
                #print(huskyX[0], " " , huskyY[0])
                change_in_heading_shepherd = numpy.arctan2(Sh[i][1] - temp_Sh[i][1],Sh[i][0] - temp_Sh[i][0]) #find the new heading using initial positon and the new postion
                print("DIRECION PRED: " , direction)
                #change_in_heading_shepherd = numpy.arctan2(direction[i][1], direction[i][1])
                ...
                sheperd_speed = 0.75
                pred_dist_from_sheep = numpy.linalg.norm(initpoints.T - Sh , axis = 1)
                no_of_sheep_within_5m = numpy.where(pred_dist_from_sheep < 1.5*(ra-1)) #3*ra as per Strombom
                print("no_of_sheep_within_5m: " , no_of_sheep_within_5m)   
                if(numpy.size(no_of_sheep_within_5m) > 0):
                    sheperd_speed = 0.55 #to prevent the predator hitting the sheep
                #if(numpy.sqrt(pow(temp_Sh[i][1]-huskyY[N1+i],2) + pow(temp_Sh[i][0]-huskyX[N1+i],2)) < 0.5):
                    #sheperd_speed = 0.0;
            ...
            #gazebo_dist_covered = numpy.sqrt((initial_gazeboY - huskyY[0])**2 + (initial_gazeboX - huskyX[0])**2)
            #initial_gazeboX = huskyX[0]; initial_gazeboY = huskyY[0];
            #if(gazebo_dist_covered < 1.5):
            #H1.linear.x = 0.75 #max distance that can be travelled is 1.5m, which is the step size
            #H1.angular.z = k*()
            #else:
            #    H1.linear.x = 0.0
            error = 0.05;
            
            normalise_angular_vel = 1; 
            #print("initial angle gazebo: " , initial_angle)   
            # if(Yaw[0] - converted_new_yaw_gazebo > error):  #initial_angle = 0 wrt to gazebo    
            #     #H1.angular.z = (Yaw[0] - converted_new_yaw_gazebo)/normalise_angular_vel
            #     H1.angular.z = 0.25
            # else:
            #     H1.angular.z = 0.0;  
            # print("angular velocity: " , H1.angular.z)      
            # initial_angle = converted_new_yaw_gazebo
            
                

            if((eaten1 == 0)): #START EATING #numpy.size(outthreshold) < 0.1*N and
                # if(N - numpy.size(outthreshold) > 0):
                #     eating1  = 1 ;    
                M.markers[N+nos].scale.x = M.markers[N+nos].scale.x - (0.02)*(N - numpy.size(outthreshold))
                M.markers[N+nos].scale.y = M.markers[N+nos].scale.y - (0.02)*(N - numpy.size(outthreshold))    
                if(M.markers[N+nos].scale.x < 0.1):
                    eaten1 = 1; #new destination
            ...
            if((eaten2 == 0)): #START EATING 2
                # if(N - numpy.size(outthreshold) > 0):
                #     eating2 = 1;
                M.markers[N+nos+1].scale.x = M.markers[N+nos+1].scale.x - 0.02*(N - numpy.size(outthreshold))
                M.markers[N+nos+1].scale.y = M.markers[N+nos+1].scale.y - 0.02*(N - numpy.size(outthreshold))
                if(M.markers[N+nos+1].scale.x < 0.1):
                    eaten2 = 1; #new destination
                    eating2 = 0;
                    destination1 = destination                    
                    eaten3 = 0 #declare them as eaten3 = 2;
                    threshold = threshold + 5
                    mode = 2; I = []

            if(numpy.size(outthreshold) < 0.1*N and eaten3 == 0): #START EATING 2
                M.markers[N+nos+2].scale.x = M.markers[N+nos+2].scale.x - (0.02)*(N - numpy.size(outthreshold))
                M.markers[N+nos+2].scale.y = M.markers[N+nos+2].scale.y - (0.02)*(N - numpy.size(outthreshold))    
                if(M.markers[N+nos+2].scale.x < 0.1):
                    eaten3 = 1; #new destination
                    #destination1 = destination
                    #eaten3 = 0                     


            # if(eating1 == 1 and (eaten1 == 0)):        
            #     #if(sqrt(pow(dest2[0] - Sh[1][0],2) + pow(dest2[1] - Sh[1][1],2)) > 3.0):
            #     #if not (Sh[1][0] >= sub_dest1[0] and Sh[1][1] >= sub_dest1[1]):
            #     #if(numpy.size(neg_y2) != 0):
            #         # m2 = ANGLE BETWEEN DESTINATION AND DESTINATION2
            #         # cur_angle2 = ANGLE B/W Sheperd2 and DESTINATION2
            #         #d2 = sqrt(pow(Sh[1][0] - destination1[0],2) + pow(Sh[1][1] - destination1[1],2))
            #         #d22 = sqrt(pow(dest2[0] - destination1[0],2) + pow(dest2[1] - destination1[1],2))
            #         #dr = d22 - d2; #PUT THE CONDITION TO CHECK DISTANCE BETWEEN dest2 and Sh and reduce that
            #         #d2 = d2 + abs(cur_angle2)
            #     S1 , a = moveSheperd(numpy.asarray(Sh[1]).reshape(1,2),numpy.asarray(sub_dest1).reshape(1,2),0.75,1)
            #     S0 , a = moveSheperd(numpy.asarray(Sh[0]).reshape(1,2),numpy.asarray(sub_dest1).reshape(1,2),0.75,1)
            #     D2=numpy.linalg.norm(initpoints.T-S1,axis=1)
            #     if(numpy.where(D2 < rs)):
            #         S1, a = moveSheperd(S1,numpy.asarray(destination1).reshape(1,2),0.75,-1)
            #         S0, a = moveSheperd(S0,numpy.asarray(destination1).reshape(1,2),0.75,-1)                    
            #     Sh[1][0] = S1[0][0]
            #     Sh[1][1] = S1[0][1]    
            #     Sh[0][0] = S0[0][0]
            #     Sh[0][1] = S0[0][1]                    

            # if(eating2 == 1 and (eaten2 == 0)):        
            #     #if(sqrt(pow(dest2[0] - Sh[1][0],2) + pow(dest2[1] - Sh[1][1],2)) > 3.0):
            #     #if not (Sh[1][0] >= sub_dest1[0] and Sh[1][1] >= sub_dest1[1]):
            #     #if(numpy.size(neg_y2) != 0):
            #         # m2 = ANGLE BETWEEN DESTINATION AND DESTINATION2
            ...
            if(count > 2000):
                ...
                not_completed[0][mc] = 1
                agent_sep[0][mc] = N - numpy.size(outthreshold)
                break    
            rate.sleep()

        #print("SSSSSSSSSSSSSSSSSSSSSSSSSSSSS")
        print("mc : " ,mc)    
        stop_time[0][mc] = rospy.get_rostime().to_sec()
        #split_diff[0][mc] = split_time[0][mc] - start_time[0][mc]
        diff[0][mc] = stop_time[0][mc] - start_time[0][mc]
        agent_sep[0][mc] = num_separated_terms
        #print("diff: " , diff)
    print("++++++++++++++++++++++++++++++++++++++")
