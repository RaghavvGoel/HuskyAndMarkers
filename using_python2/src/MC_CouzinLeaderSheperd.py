...
def SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,leader_pos,leaderRrep,leaderRori,leaderRatt,destination,destinationthreshold,nol,leader_vels,destination_other):
    pos= initpoints.T
    #print("pos:")
    #print(pos)
    #print("SH=" , S , " shape: " , numpy.shape(S))
    if(numpy.shape(S)[0] == 0):
        leader_idx = [i+N for i in range(6)]
        nos = 0;
    else:
...
def main():
    ...
    # destination1x = [ -8., -19., -17., -92.,-73., -14., -60., -97., -63., -67., -85., -31., -51., -76., -61.,-46., -80., -22.]

    # destination1y = [ -70., -66.,  -2., -66.,-62., -62., -89., -26., -95., -66., -28., -18., -42.,  -6., -98.,-55., -67., -91.]

    # destination2x = [ 98.,  92.,  96.,  97.,63.,  73.,  76.,  55.,  72.,  25.,  40.,  58.,  90.,  23.,  95.,57.,  88.,  59.]

    # destination2y = [   4.,  94.,  53.,  16.,80.,  42.,   0.,  19.,  46.,  74.,  36.,  55.,  47.,  89.,  92.,52.,  39.,  27.]

    # destinationx = [   70.,  65., -51., -37.,38.,  39.,  31.,  30.,  36.,   0.,   9.,  96.,  52.,  66.,  43.,-84.,  34.,  95.]

    # destinationy  = [ 74., -29.,  88.,  62.,-98.,  87.,  87.,  87., -32.,   7.,  89.,  86.,  91.,  55.,  53.,99.,  69.,  70.]        
    ...
    for mc1 in range(MC):
        ...
        leader_pos4 = [0.5,-45.0]
        leader_pos6 = [1.5,-45.0]

        #leader_pos = numpy.asarray(leader_pos1).reshape(nol,2)
        #LeaderPos = numpy.asarray([leader_pos1,leader_pos2]).reshape(nol,2)
        #LeaderPos = numpy.asarray([leader_pos1,leader_pos2,leader_pos3,leader_pos4]).reshape(nol,2)
        LeaderPos = numpy.asarray([leader_pos1,leader_pos2,leader_pos3,leader_pos4,leader_pos5,leader_pos6]).reshape(nol,2)
        ...
        leader_vels1 = numpy.zeros((3,2)); 
        leader_vels2 = numpy.zeros((3,2))        
        leader_vels1[0][0] = numpy.random.rand(1,1)    
        leader_vels1[0][1] = numpy.random.rand(1,1)    
        leader_vels1[1][0] = numpy.random.rand(1,1)    
        leader_vels1[1][1] = numpy.random.rand(1,1)    
        leader_vels1[2][0] = numpy.random.rand(1,1)    
        leader_vels1[2][1] = numpy.random.rand(1,1)    


        leader_vels2[0][0] = numpy.random.rand(1,1)    
        leader_vels2[0][1] = numpy.random.rand(1,1)    
        leader_vels2[1][0] = numpy.random.rand(1,1)    
        leader_vels2[1][1] = numpy.random.rand(1,1)            
        leader_vels2[2][0] = numpy.random.rand(1,1)    
        leader_vels2[2][1] = numpy.random.rand(1,1)                    
        ...
        swarm_split = 0;
        has_oriented1 = 0 ; has_oriented2 = 0 ;
        ...
        while not rospy.is_shutdown():
            ...
            if(((near_swarm1 == 1 and near_swarm2 == 1) or swarm_split == 1) and (eaten1 == 0 or eaten2 == 0)):
                ...
                if(orient1 == 0 or orient2 == 0):
                    ...
                    if(entered_once == 0):
                        ...
                        sub_dest1[0] = GCM0[0] + 7.5*numpy.cos(m1)
                        sub_dest1[1] = GCM0[1] + 7.5*numpy.sin(m1)

                        m2 = numpy.arctan2(destination2[0][1]-GCM1[1],destination2[0][0]-GCM1[0])
                        sub_dest2[0] = GCM1[0] + 7.5*numpy.cos(m2)
                        sub_dest2[1] = GCM1[1] + 7.5*numpy.sin(m2)
                        entered_once = 1;
                    print("HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH")    
                    print("sub_dest1: " , sub_dest1 , " sub_dest2: " , sub_dest2)
                    initpoints,vels,notused,notused =  SheepMovement(initpoints,vels,Rrep,Rrep,Ratt,s1,dt,phi,omega,LeaderPos,rs,[],leaderRrep,leaderRori,leaderRatt,numpy.asarray([0,0]).reshape(1,2),0,0,[],numpy.asarray([0,0]).reshape(1,2))    

                else:    
                    initpoints,vels,LeaderPos,LeaderVels =  SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,LeaderPos,leaderRrep,leaderRori,leaderRatt,destination1,destinationthreshold,nol,LeaderVels,destination2)
                    if(numpy.linalg.norm((LeaderPos[0] - destination1[0]),axis = 0) <= destinationthreshold  and in1 == 1 and entered1 == 0):
                        entered1 = 1;
                        GCM0 = [numpy.average(initpoints[0,ind_gcm1[0]]),numpy.average(initpoints[1,ind_gcm1[0]])]
                        m1 = numpy.arctan2((destination[0][1] - GCM0[1]),(destination[0][0] - GCM0[0]))
                        # if(m1 > pi/2):
                        #     m1 = m1 - pi/2
                        dest1[0][0] = GCM0[0] + (5)*numpy.cos(m1);
                        dest1[0][1] = GCM0[1] + 5*numpy.sin(m1);    


                    if(numpy.linalg.norm((LeaderPos[1] - destination2[0]),axis = 0) <= destinationthreshold and in2 == 1 and entered2 == 0):
                        entered2 = 1;
                        GCM1 = [numpy.average(initpoints[0,ind_gcm2[0]]),numpy.average(initpoints[1,ind_gcm2[0]])]
                        m2 = numpy.arctan2((destination[0][1] - GCM1[1]),(destination[0][0] - GCM1[0]))
                        # if(m2 > pi/2):
                        #     m2 = m2 - pi/2
                        dest2[0][0] = GCM1[0] + (5)*numpy.cos(m2);
                        dest2[0][1] = GCM1[1] + 5*numpy.sin(m2);    
                ...
                initpoints,vels,notused,notused =  SheepMovement(initpoints,vels,Rrep,Rrep,Ratt,s1,dt,phi,omega,LeaderPos,rs,[],leaderRrep,leaderRori,leaderRatt,numpy.asarray([0,0]).reshape(1,2),0,nol,[],numpy.asarray([0,0]).reshape(1,2))        

            
            elif(eaten1 == 1 and eaten2 == 1):
                # if(entered_once2 == 0):
                #     #FIND THE NEW POS of LEADERS : 
                ...
                initpoints,vels,LeaderPos,LeaderVels =  SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,LeaderPos,leaderRrep,leaderRori,leaderRatt,destination,destinationthreshold+5,nol,LeaderVels,destination)                        
            ...
            for i in range(N + nol):
                ...
                if(i < N):
                    ...
                    #print(leader_pos1)
                    M.markers[i].pose.position.x = LeaderPos[i-N][0]
                    M.markers[i].pose.position.y = LeaderPos[i-N][1]
            ...
            if(near_swarm1 == 0 and swarm_split == 0):
                ...
                sp1 = numpy.linalg.norm(LeaderPos[0] - GCM)*(0.2)                                                           
                #print("sp1= " , sp1)         
            ...
            if(near_swarm1 == 0 and swarm_split == 0):
                D3 = numpy.linalg.norm(initpoints.T - LeaderPos[2],axis=1)
                sp1 = numpy.linalg.norm(LeaderPos[2] - GCM)*(0.2)                                                           
                #print("sp1= " , sp1)         
                idxD3 = numpy.where(D3 < rs/3.0)            
                if(numpy.size(idxD3) > 0):
                    near_swarm1 = 1;
                else:
                    LeaderPos[2],a = moveLeader(LeaderPos[2],GCM,sp1*dt,1)                    

            if(near_swarm1 == 0 and swarm_split == 0):
                D5 = numpy.linalg.norm(initpoints.T - LeaderPos[4],axis=1)
                sp1 = numpy.linalg.norm(LeaderPos[4] - GCM)*(0.2)                                                           
                #print("sp1= " , sp1)         
                idxD5 = numpy.where(D5 < rs/3.0)            
                if(numpy.size(idxD5) > 0):
                    near_swarm1 = 1;
                else:
                    LeaderPos[4],a = moveLeader(LeaderPos[4],GCM,sp1*dt,1)                                        

            
            if(near_swarm2 == 0 and swarm_split == 0):
                D2 = numpy.linalg.norm(initpoints.T - LeaderPos[1],axis=1)
                sp2 = numpy.linalg.norm(LeaderPos[1] - GCM)*(0.2)
                #print(" sp2= " , sp2)
            ...
            if(near_swarm2 == 0 and swarm_split == 0):
                D4 = numpy.linalg.norm(initpoints.T - LeaderPos[3],axis=1)
                sp1 = numpy.linalg.norm(LeaderPos[3] - GCM)*(0.2)                                                           
                #print("sp1= " , sp1)         
                idxD4 = numpy.where(D4 < rs/3.0)            
                if(numpy.size(idxD4) > 0):
                    near_swarm2 = 1;
                else:
                    LeaderPos[3],a = moveLeader(LeaderPos[3],GCM,sp1*dt,1)            

            if(near_swarm2 == 0 and swarm_split == 0):
                D5 = numpy.linalg.norm(initpoints.T - LeaderPos[5],axis=1)
                sp1 = numpy.linalg.norm(LeaderPos[5] - GCM)*(0.2)                                                           
                #print("sp1= " , sp1)         
                idxD5 = numpy.where(D5 < rs/3.0)            
                if(numpy.size(idxD5) > 0):
                    near_swarm2 = 1;
                else:
                    LeaderPos[5],a = moveLeader(LeaderPos[5],GCM,sp1*dt,1)            


            
            if(abs(LeaderPos[0][1] - LeaderPos[1][1]) < 10 and swarm_split == 0):
                swarm_split = 1;        
            ...
            if(orient1 == 0 and swarm_split == 1 and entered_once == 1):
                ...
                D1 = numpy.linalg.norm(initpoints.T - LeaderPos[0],axis=1)
                idx_D1 = numpy.where(D1 < 5) #rs/2.0
                if(numpy.size(idx_D1) > 0):
                    LeaderPos[0],a = moveLeader(LeaderPos[0],numpy.asarray([initpoints[:,idx_D1[0][0]]]).reshape(1,2),1,-1.25)

            if(orient1 == 0 and swarm_split == 1 and entered_once == 1):
                #print("GCM0: " , GCM0 , " LeaderPos: " , LeaderPos[0])
                LeaderPos[2],a = moveLeader(LeaderPos[2],sub_dest1,1,1)
                D1 = numpy.linalg.norm(initpoints.T - LeaderPos[2],axis=1)
                idx_D1 = numpy.where(D1 < 5) #rs/2.0
                if(numpy.size(idx_D1) > 0):
                    LeaderPos[2],a = moveLeader(LeaderPos[2],numpy.asarray([initpoints[:,idx_D1[0][0]]]).reshape(1,2),1,-1.25)
                if(numpy.linalg.norm(LeaderPos[0] - LeaderPos[2]) < 5):
                    LeaderPos[2],a = moveLeader(LeaderPos[2],numpy.asarray(LeaderPos[0]).reshape(1,2),1.15,-1)                    

            if(orient1 == 0 and swarm_split == 1 and entered_once == 1):
                #print("GCM0: " , GCM0 , " LeaderPos: " , LeaderPos[0])
                LeaderPos[4],a = moveLeader(LeaderPos[4],sub_dest1,1,1)
                D1 = numpy.linalg.norm(initpoints.T - LeaderPos[4],axis=1)
                idx_D1 = numpy.where(D1 < 5) #rs/2.0
                if(numpy.size(idx_D1) > 0):
                    LeaderPos[4],a = moveLeader(LeaderPos[4],numpy.asarray([initpoints[:,idx_D1[0][0]]]).reshape(1,2),1,-1.25)
                if(numpy.linalg.norm(LeaderPos[0] - LeaderPos[4]) < 5):
                    LeaderPos[4],a = moveLeader(LeaderPos[4],numpy.asarray(LeaderPos[0]).reshape(1,2),1.15,-1)
                if(numpy.linalg.norm(LeaderPos[2] - LeaderPos[4]) < 5):
                    LeaderPos[4],a = moveLeader(LeaderPos[4],numpy.asarray(LeaderPos[2]).reshape(1,2),1.15,-1)                                                            
            ...
            if(orient2 == 0 and swarm_split == 1 and entered_once == 1):
                ...
                D2 = numpy.linalg.norm(initpoints.T - LeaderPos[1],axis=1)
                idx_D2 = numpy.where(D2 < 5) #rs/2
                if(numpy.size(idx_D2) > 0):
                    LeaderPos[1],a = moveLeader(LeaderPos[1],numpy.asarray([initpoints[:,idx_D2[0][0]]]).reshape(1,2),1,-1.25)                

            if(orient2 == 0 and swarm_split == 1 and entered_once == 1):
                print("GCM0: " , GCM0 , " LeaderPos: " , LeaderPos[0])
                LeaderPos[3],a = moveLeader(LeaderPos[3],sub_dest2,1,1)
                D2 = numpy.linalg.norm(initpoints.T - LeaderPos[3],axis=1)
                idx_D2 = numpy.where(D1 < 5) #rs/2.0
                if(numpy.size(idx_D2) > 0):
                    LeaderPos[3],a = moveLeader(LeaderPos[3],numpy.asarray([initpoints[:,idx_D2[0][0]]]).reshape(1,2),1,-1.25)
                if(numpy.linalg.norm(LeaderPos[1] - LeaderPos[3]) < 5):
                    LeaderPos[3],a = moveLeader(LeaderPos[3],numpy.asarray(LeaderPos[1]).reshape(1,2),1.15,-1)   

            if(orient2 == 0 and swarm_split == 1 and entered_once == 1):
                print("GCM0: " , GCM0 , " LeaderPos: " , LeaderPos[0])
                LeaderPos[5],a = moveLeader(LeaderPos[5],sub_dest2,1,1)
                D2 = numpy.linalg.norm(initpoints.T - LeaderPos[5],axis=1)
                idx_D2 = numpy.where(D1 < 5) #rs/2.0
                if(numpy.size(idx_D2) > 0):
                    LeaderPos[5],a = moveLeader(LeaderPos[5],numpy.asarray([initpoints[:,idx_D2[0][0]]]).reshape(1,2),1,-1.25)
                if(numpy.linalg.norm(LeaderPos[1] - LeaderPos[5]) < 5):
                    LeaderPos[5],a = moveLeader(LeaderPos[5],numpy.asarray(LeaderPos[1]).reshape(1,2),1.15,-1) 
                if(numpy.linalg.norm(LeaderPos[3] - LeaderPos[5]) < 5):
                    LeaderPos[5],a = moveLeader(LeaderPos[5],numpy.asarray(LeaderPos[3]).reshape(1,2),1.15,-1)                                           

            #Let's check whether leaders come in beteen the swarm and trhe destiantion or not
            if(swarm_split == 1 and entered_once == 1):
                #if(abs(destination1[0][0] - GCM0[0]) > abs(LeaderPos[0][0] - GCM0[0]) or abs(destination1[0][1] - GCM0[1]) > abs(LeaderPos[0][1] - GCM0[1])):
                dist_from_sub_dest1 = numpy.linalg.norm(sub_dest1 - LeaderPos[0])
                if(dist_from_sub_dest1 < 6):
                    has_oriented1 = 1;
                    orient1 = 1
            if(swarm_split == 1 and entered_once == 1):
                #if(abs(destination2[0][0] - GCM1[0]) > abs(LeaderPos[1][0] - GCM1[0]) or abs(destination2[0][1] - GCM1[1]) > abs(LeaderPos[1][1] - GCM1[1])):
                dist_from_sub_dest2 = numpy.linalg.norm(sub_dest2 - LeaderPos[1])
                if(dist_from_sub_dest2 < 6):                    
                    has_oriented2 = 1; 
                    orient2 = 1

            # if(swarm_split==1 and entered_once == 1 and has_oriented1 == 1 and orient1 == 0):
            #     print("PPPPPPPPPPPPPPPPPPPPPPPPP")
            #     orient1 = 1;        
            # if(swarm_split==1 and entered_once == 1 and has_oriented2 == 1 and orient2 == 0):
            #     orient2 = 1;                    
            ...
            g1 = numpy.where(dist1 < dist2)
            g2 = numpy.where(dist2 < dist1)
            temp_g1 = copy.deepcopy(g1)
            temp_g2 = copy.deepcopy(g2)
            #print("g1: " , g1)
            #print("g2: " , g2)
            ...
            if((near_swarm1 == 1 and near_swarm2 == 1) or swarm_split == 1):
                ...
                if(in1 == 1 and eaten1 == 0):
                    ...
                    M.markers[N+nol].scale.x = M.markers[N+nol].scale.x - size_dec
                    M.markers[N+nol].scale.y = M.markers[N+nol].scale.y - size_dec
                    if(M.markers[N+nol].scale.x < 0.1):
                        eaten1 = 1; 

                if(in2 == 1 and eaten2 == 0):
                    M.markers[N+nol+1].scale.x = M.markers[N+nol+1].scale.x - size_dec
                    M.markers[N+nol+1].scale.y = M.markers[N+nol+1].scale.y - size_dec
            ...
            if(entered1 == 1 and (eaten1 == 0 or Lreached1 == 0)):
                dist_from_dest1 = numpy.linalg.norm(dest1 - LeaderPos[0])
                #if((LeaderPos[0][0] < dest1[0][0] or LeaderPos[0][1] < dest1[0][1])):
                if(dist_from_dest1 > 5):
                    ...
                    LeaderPos[2],a = moveLeader(LeaderPos[2],dest1,speed,1) 
                    D_from_L1 = numpy.linalg.norm(initpoints.T - LeaderPos[2],axis =1)      
                    close_to_L1 = numpy.where(D_from_L1 < leaderRrep/2.0)
                    if(numpy.size(close_to_L1) > 0):
                        LeaderPos[2],a = moveLeader(LeaderPos[2],destination1,speed,-1)
                    if(numpy.linalg.norm(LeaderPos[0] - LeaderPos[2]) < 3):
                        LeaderPos[2],a = moveLeader(LeaderPos[2],numpy.asarray(LeaderPos[0]),speed,-1) 

                    LeaderPos[4],a = moveLeader(LeaderPos[4],dest1,speed,1) 
                    D_from_L1 = numpy.linalg.norm(initpoints.T - LeaderPos[4],axis =1)      
                    close_to_L1 = numpy.where(D_from_L1 < leaderRrep/2.0)
                    if(numpy.size(close_to_L1) > 0):
                        LeaderPos[4],a = moveLeader(LeaderPos[4],destination1,speed,-1)
                    if(numpy.linalg.norm(LeaderPos[0] - LeaderPos[4]) < 3):
                        LeaderPos[4],a = moveLeader(LeaderPos[4],numpy.asarray(LeaderPos[0]),speed + 0.15,-1)
                    if(numpy.linalg.norm(LeaderPos[2] - LeaderPos[4]) < 3):
                        LeaderPos[4],a = moveLeader(LeaderPos[4],numpy.asarray(LeaderPos[2]),speed + 0.15,-1)   
                else:
                    Lreached1 = 1;    

            if(entered2 == 1 and (eaten2 == 0 or Lreached2 == 0)):
                dist_from_dest2 = numpy.linalg.norm(dest2 - LeaderPos[1])
                #if((LeaderPos[1][0] > dest2[0][0] or LeaderPos[1][1] < dest2[0][1])):
                if(dist_from_dest2 > 5):
                    ...
                    print("dest2= " , dest2 ," " , dest2[0][1], " m2= " , m2 , "sin(m2)= ", numpy.sin(m2))
                    print("GCM1: " , GCM1)    

                    LeaderPos[3],a = moveLeader(LeaderPos[3],dest2,speed,1) 
                    D_from_L2 = numpy.linalg.norm(initpoints.T - LeaderPos[3],axis =1)      
                    close_to_L2 = numpy.where(D_from_L2 < leaderRrep/2.0)
                    if(numpy.size(close_to_L2) > 0):
                        LeaderPos[3],a = moveLeader(LeaderPos[3],destination1,speed,-1)
                    if(numpy.linalg.norm(LeaderPos[1] - LeaderPos[3]) < 3):
                        LeaderPos[3],a = moveLeader(LeaderPos[3],numpy.asarray(LeaderPos[1]),speed,-1)

                    LeaderPos[5],a = moveLeader(LeaderPos[5],dest2,speed,1) 
                    D_from_L2 = numpy.linalg.norm(initpoints.T - LeaderPos[5],axis =1)      
                    close_to_L2 = numpy.where(D_from_L2 < leaderRrep/2.0)
                    if(numpy.size(close_to_L2) > 0):
                        LeaderPos[5],a = moveLeader(LeaderPos[5],destination1,speed,-1)
                    if(numpy.linalg.norm(LeaderPos[1] - LeaderPos[5]) < 3):
                        LeaderPos[5],a = moveLeader(LeaderPos[5],numpy.asarray(LeaderPos[1]),speed+0.15,-1) 
                    if(numpy.linalg.norm(LeaderPos[3] - LeaderPos[5]) < 3):
                        LeaderPos[5],a = moveLeader(LeaderPos[5],numpy.asarray(LeaderPos[3]),speed0+ 0.15,-1)                                                                                                    
                else:
                    Lreached2 = 1;    
                ...
                #flag = 1;
                #break        
            if(eaten1 == 1 and eaten2 == 1 and eating3 == 1 and eaten3 == 0):
                #towrds destination3 
                M.markers[N+nol+2].scale.x = M.markers[N+nol+2].scale.x - size_dec
                M.markers[N+nol+2].scale.y = M.markers[N+nol+2].scale.y - size_dec
                if(M.markers[N+nol+2].scale.x < 0.1):
                    eaten3 = 1;     
            ...
            if(count > 4000):
                ...
                if(eaten1 != 1 or eaten2 != 0):
                    agent_sep[0][mc1] = numpy.size(temp_g1) - numpy.size(near_dest1) + numpy.size(temp_g2) - numpy.size(near_dest2)
                    if(numpy.size(temp_g1) - numpy.size(near_dest1) > 0):
                        task[0][mc1] = 1;
                    if(numpy.size(temp_g2) - numpy.size(near_dest2) > 0):
        ...
        split_diff[0][mc1] = split_time[0][mc1] - start_time[0][mc1] #will be same as previous 
