...
def main():
    ...
    pub_marker = rospy.Publisher('rover_sheep_sheperd', MarkerArray , queue_size=1)
    ...
    for i in range(N+nos+1): #to include shepherd too
        marker = Marker()
        print("i " , i)
        marker.header.frame_id = "/rover_sheep_sheperd";
    ...
    while not rospy.is_shutdown():
        ...
        #if(lat != None):
        d = 2*(RadOfE)*numpy.arcsin(sqrt(pow(sin((lat-ini_lat)/2),2)) + cos(ini_lat)*cos(lat)*pow(sin((lon-ini_lon)/2),2))
        d = d*10.0
        #d=cos((lat-ini_lat)/2)('vlaue of d: ', 0.55682866656587993, ' slope: ', -1.107148717807825)

        slope = numpy.arctan2(lon - ini_lon , lat - ini_lat)
        print("vlaue of d: " , d , " slope: " , slope)
        shep_x = d*cos(slope)
        shep_y = d*sin(slope)
        Sh[0,0] = shep_x
        Sh[0,1] = shep_y
        #temp_x = (111415.13 * cos(lat))- (94.55 * cos(3.0*lat)) + (0.12 * cos(5.0*lat))
        #temp_y = (111132.09 - (566.05 * cos(2.0*lon))+ (1.20 * cos(4.0*lon)) - (0.002 * cos(6.0*lon)))
        print("LAT and LON: " , lat - ini_lat , " " , lon - ini_lon , " " , ini_lat , " " , ini_lon)
        ...
        V =  sqrt(pow(dx,2) + pow(dy,2))/(0.1)
        Omega = numpy.arctan2(dy,dx)/(0.1)
