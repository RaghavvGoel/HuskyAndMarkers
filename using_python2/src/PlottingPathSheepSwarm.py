    ...
    for j in range(Shepherdsize[0]):
        ...
        if(leader_on == 0):    
            ...
            for i in range(numpy.size(closeagents)):
                ...
                D_less_than_10 = numpy.where((orderedD < 5.0))
                #if(swarm_split != 1):
...
def main():
    rospy.init_node('sheep_sheperd_plot', anonymous=True)
    ...
    N = 60
    x_path = numpy.zeros((N,C))
    y_path = numpy.zeros((N,C))
    Sh0_path = numpy.zeros((2,C))
    Sh1_path = numpy.zeros((2,C))
    for mc2 in range(MC):
        for mc in range(1):
            ...
            step = 1.5
            shape = Marker.CUBE;
            for i1 in range(N+nos): #to include shepherd too
                marker = Marker()
                print("i1 " , i1)
            ...
            while not rospy.is_shutdown():
                ...
                for i in range(N):
                    ...
                    x_path[i][count] = initpoints[0][i]
                    y_path[i][count] = initpoints[1][i]
                for i in range(nos):    
                    ...
                    if(i == 0):
                        Sh0_path[0][count] = Sh[i][0]
                        Sh0_path[1][count] = Sh[i][1]
                    if(i == 1):
                        Sh1_path[0][count] = Sh[i][0]
                        Sh1_path[1][count] = Sh[i][1]                        
                    if(nos > 1):
                        #toggle = toggle*(-1)
                        switch = switch*(-1)
                ...
                if(eating2 == 1 and out2 == 0 and nos > 1):
                ...
                if(eating2 == 1 and pos_update2 == 0 and nos > 1):        
                ...
                if(leader_front2 == 0 and orient1 == 1 and nos > 1):
                ...
                # print("Sh[0]: " , Sh[0])
                #print("D1: " , D1)
                if(nos > 1 and abs(Sh[0][1] - Sh[1][1]) < rs):
                    s = 0.0    
                elif((n2 > 0 or n3 > 0) and crossed_swarm == 0):
                    s = 0.1 + mc2*0.05
                    #################################################################
                else:
                    s = 0.75
                ...
                if(n2 > 0 or n3 > 0):
                    ...
                    sort_grp1 = numpy.argsort(initpoints[0][grp1]) # now nearest to 0 is in the last
                    sort_grp2 = numpy.argsort(initpoints[0][grp2]) # nearest is at index 0
                    negx = max(initpoints[0][grp1])
                    negy = numpy.where(initpoints[0][grp1] == negx)
                    posx = min(initpoints[0][grp2])
                    posy = numpy.where(initpoints[0][grp2] == posx)
                    #print("sort_grp2: " , sort_grp2, " " , sort_grp2[0] , " " ,initpoints[0][sort_grp2[0]])
                    #print("grp2: " , grp2)
                    #print("posx: " , posx ,  " posy: " , posy)
                    #print("sort_grp1: " , sort_grp1 , " " , sort_grp1[len(grp1)-1] , " " , initpoints[0][sort_grp1[len(grp1)-1]])
                    #print("grp1: " , grp1)
                    #print("negx: " , negx , " negy: " , negx)
                    #d = numpy.sqrt(pow(initpoints[1][sort_grp1[len(grp1)-1]] - initpoints[1][sort_grp2[0]],2) + pow(initpoints[0][sort_grp1[len(grp1)-1]] - initpoints[0][sort_grp2[0]],2))
                    #d = numpy.sqrt(pow(posy[0] - negy , 2)+ pow(posx - negx , 2))
                    d = abs(posx - negx)
                    print("d: " , d)
                    if(d > 15):
                ...
                # if(nos > 1 and Sh[0][1] <= Sh[1][1] and enter_once == 0):
                #     switch = 0;
                #     toggle = 1;
                #     #crossed_swarm = 1;
                #     enter_once = 1;
            ...
            time[mc2][mc] = stop_time - start_time
    ...
    for i in range(MC):
        print("speed = " , 0.1 + i*0.05 , "time: " , numpy.sum(time[i])/100 , "  ratio: " , numpy.sum(ratio[i])/(N*100 - numpy.sum(ratio[i])))
            

    x_axis = [i for i in range(1000)]
    
    for i in range(numpy.size(grp1)):
        #if(i in grp1):
        T1 = plt.plot(x_path[grp1[i]], y_path[grp1[i]] ,color = ( (0.0,1.0,0.0)) )        
    for i in range(numpy.size(grp2)):   
        T2 = plt.plot(x_path[grp2[i]], y_path[grp2[i]] ,color = ( (0.5, 0.5, 0.0) ))        
    #print("difference: " , stop_time - start_time)    
    #plt.plot(x_axis,Sh0_path,color = ( (1.0, 0.0, 0.0) ) , label = "Predator" )        
    #plt.plot(x_axis,Sh1_path,color = ( (0.0, 0.0, 1.0) ) , label = "Predator")
    T3 = plt.plot(Sh0_path[0],Sh0_path[1], 'r' , Sh1_path[0],Sh1_path[1], 'r' , linewidth = 2 )
    #plt.legend(T1[0] , T2[0] , ('agents' , 'agents'))        
    plt.legend(T3[:1] , ['predators'])
    plt.show()
