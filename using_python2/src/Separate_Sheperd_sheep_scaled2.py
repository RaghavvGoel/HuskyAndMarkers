...
def callback(msg):
    ...
    global X , Y, N , N1 , FLAG , initpoints , x_out , y_out
...
def main():
    ...
    rmove_sub_local1 = rospy.Subscriber('rover1/mavros/global_position/local', Odometry, get_position_local)
    #rvel = rospy.Publisher('mavros/setpoint_raw/local', PositionTarget, queue_size = 1)
    rvel1 = rospy.Publisher('rover1/mavros/setpoint_velocity/cmd_vel' , TwistStamped ,queue_size = 1)
    rcompass1 = rospy.Subscriber('rover1/mavros/global_position/compass_hdg', Float64 , get_compass)
    
    rmove_sub_local2 = rospy.Subscriber('rover2/mavros/global_position/local', Odometry, get_position_local)
    #rvel = rospy.Publisher('mavros/setpoint_raw/local', PositionTarget, queue_size = 1)
    rvel2 = rospy.Publisher('rover2/mavros/setpoint_velocity/cmd_vel' , TwistStamped ,queue_size = 1)
    rcompass2 = rospy.Subscriber('rover2/mavros/global_position/compass_hdg', Float64 , get_compass)
    ...
    while(heading_angle2 < 85 or heading_angle2 > 95):
        print("in while " , heading_angle2)            
        rov1.twist.linear.x = 0
    ...
    while not rospy.is_shutdown():
        ...
        Sh[0,0] = ini_shep[0][0] + local_lat1 
        Sh[0,1] = ini_shep[0][1] + local_lon1
        Sh[1,0] = ini_shep[1][0] + local_lat2 
        Sh[1,1] = ini_shep[1][1] + local_lon2 
        ...
        sheepmovements(initpoints,n,c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode,destination,p,dist_threshold)
        ...
        dx2 = (Sh[0,0] - temp_sh[0,0])
        dy2 = (Sh[0,1] - temp_sh[0,1])
        V2 =  sqrt(pow(dx2,2) + pow(dy2,2))
        theta2 = numpy.arctan2(dy2,dx2)        
        #rov.position.x = ini_lat + dx
        #rov.position.y = ini_lon + dy
        print("V2: " , V2 , " " , theta2)
        rov2.twist.linear.x = V2        
        ...
        while(heading_angle2 < beta2 - 10 or heading_angle2 > beta2 + 10):
            print("beta2: " , beta2 , "heading_angle2 " , heading_angle2)            
            ...
            rov2.twist.angular.z = yaw_to_publish         
            rvel2.publish(rov2)
            r2flag.publish(flag2_inwhile)


        flag2_inwhile = False    
        time1 = rospy.get_rostime()
        while(rospy.get_rostime() < time1 + rospy.Duration(1.5)):
            print("in time loop")
            rov2.twist.linear.x = 12.0
            rov2.twist.angular.z = 00.0
            rvel2.publish(rov1)
            r2flag.publish(flag1_inwhile)
