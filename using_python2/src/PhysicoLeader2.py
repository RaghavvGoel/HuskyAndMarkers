...
def initPhysico(initpoints,V1, deltaT,leaderidx,rs,dps,rhos,ra,destination,destinationthreshold):
    ...
    if(numpy.size(idxgravity) != 0):
        ...
        for i in range(numpy.shape(D)[0]):
            ...
            for k in range(numpy.shape(d)[0]):
                #scalar[k] = pow(G*m*m*d[k],-(p+1)) # = G*d[k])^(-3) = 1/(56.25*D[k])^-3 
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        initpoints,V1 = initPhysico(initpoints,V1, deltaT,leaderidx,rs,dps,rhos,ra,destination,destinationthreshold)
        print("yolo")
        #print("x: " , x)
        #print("y: " , y)
        for i in range(N + nos):
            ...
            if(i < N):
                ...
                sp = 0.05
                S,a = moveSheperd(initpoints[:,leaderidx[0]],copy.deepcopy(destination),sp,1)
                initpoints[0][leaderidx[0]] = S[0][0]
                initpoints[1][leaderidx[0]] = S[0][1]
                #M.markers[i].pose.position.y = initpoints[1][leaderidx[0]] 
        ...
        near_dest = numpy.where(dist <= destinationthreshold)
        print("destination: " , destination)
        print("initpoints: " , initpoints.T)
        print("dist: " , dist)
        print("near_dest: " , near_dest)
