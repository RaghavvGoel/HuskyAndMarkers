    ...
    while not rospy.is_shutdown():
        ...
        if(toggle == 0):
            ...
            if(len(g1) > 0.1*(numpy.size(grp1)) and eaten1 == 0):
                print("GRP1")
                sheepmovements(initpoints,(0.9*(len(g1))),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode1,destination1,p,swarm_split)
                ...
                dest1[1] = destination1[1] + 8.0*numpy.sin(alpha1)
            ...
            if(len(g2) > 0.1*(numpy.size(grp2)) and eaten2 == 0):
                print("GRP2")
                sheepmovements(initpoints,(0.9*(len(g2))),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode2,destination2,p,swarm_split)
            ...
            if(out1 == 1 and out2 == 1):
                ...
                #     #out3 = 1;
                if(len(g1) > 0.1*len(grp1) and in1 == 0):
                    sheepmovements(initpoints,0.9*len(g1),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode1,destination,p,swarm_split)
                    S0,mode1,I1 = sheperdMovement(Sh[0].reshape(1,2),initpoints[0][g1],initpoints[1][g1],ra,Pd,Pc,destination,f,mindisShep,rhoash,alpha,ang,mode1,nin,ds,rs,method,predmasterslave,decrad,q,I1)      
                else:
                    in1 = 1;   

                if(len(g2) > 0.1*len(grp2) and in2 == 0):
                    sheepmovements(initpoints,0.9*len(g2),c,rs,Sh,ra,rhoa,rhos,unitvec,h,e,dps,mode2,destination,p,swarm_split)
                    S1,mode2,I2 = sheperdMovement(Sh[1].reshape(1,2),initpoints[0][g2],initpoints[1][g2],ra,Pd,Pc,destination,f,mindisShep,rhoash,alpha,ang,mode2,nin,ds,rs,method,predmasterslave,decrad,q,I1)                      
                else:    
                    in2 = 1;
        ...
        if(eaten1 == 1 and out1 == 0):
            ...
            # if(abs(m1 - cur_angle1) > pi/10):
            #     new_pos1[0] = destination1[0] - 10.0*numpy.cos(cur_angle1 - pi/18)
            #     new_pos1[1] = destination1[1] - 10.0*numpy.sin(cur_angle1 - pi/18)
        ...
        if(eaten2 == 1 and out2 == 0):
            ...
            # if(abs(m2 - cur_angle2) > pi/10):
            #     new_pos2[0] = destination2[0] - 10.0*numpy.cos(cur_angle2)
            #     new_pos2[1] = destination2[1] - 10.0*numpy.sin(cur_angle2)
