...
def SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,leader_pos,leaderRrep,leaderRori,leaderRatt,destination,destinationthreshold,nol,leader_vels,destination_other,leader_idx):
    ...
    idx21 = numpy.where(dist > destinationthreshold - 5)
        ...
        if(flag != -1):
            ...
            if(nagentFlag > 0):
                ...
                if(i < N):
                    ...
                    dtheta = numpy.arccos(numpy.dot(di,vels[i]))
...
def main():
    ...
    destination1x = [ -82. , -78. ,-116. , -63. , -96., -119. , -71.,  -109. , -62. ,-105. , -91. ,-142. , -68. ,-128. ,-133. ,-136. , -80. ,-128. , -59.,
  -130.]
    destination1y =  [  53., -185.,   78.,   90.,   55., -192. ,  68.,
  -167., -147. , -91. , 173. ,  34. , 105. , -22., -146. , -95.  , -5. , 225. , 205.,
   -44.]

#('dest2: ',)
    destination2x = [ 171.,  223.,  187.,  240. , 212. , 232. , 239.,
   196. , 248. , 176.  ,243. , 157. , 220. , 179. , 151. , 197. , 196. , 164. , 240.,
   230.]
    destination2y = [ -223. ,-209. ,-141. , 145. , 138. ,  37. ,-162.,
    67. ,  80. , 249. ,  48. ,  42. ,  62.,  103. , 125. ,-151.,  186. , 187., -222.,
  -100.]

#('dest: ',)
    destinationx = [   54. ,  11. ,  72.,    7. , -92. ,  47. , -85.,
   -56.  , 40. , -24. , -26. ,  15. ,   7. , -71. ,  -8.,   61. ,  57. ,  21.,  -38.,
    74.]
    destinationy = [ -209. ,  31. , -59. ,-171.  , -6. ,-180. , -20.,
   ...
   142.]
    ...
    for mc1 in range(MC):
        ...
        while not rospy.is_shutdown():
            ...
            ##print("destination1: " , destination1 , " destination2: " , destination2)
            ...
            if(swarm_split == 1):
                ...
                grp1 = numpy.setdiff1d(grp1[0],separated_agents)
                grp2 = numpy.setdiff1d(grp2[0],separated_agents)
            ...
            if((eaten1 == 0)):
                ...
                #else:    
                if(entered_once2 == 0): #LEADERS FORMED
                    ...
                    ind_l = numpy.random.choice(N,2)
                    #leader_idx = [ind_gcm1[0][ind_l1[0]],ind_gcm2[0][ind_l2[0]]]
                    #print("ind_l: " , ind_l)
                    # leader_idx = [ind_l[0],ind_l[1]]
                    # #print("leader_idx: " , leader_idx)
                    ...
                    for k in range((N)):
                        ...
                        interD = numpy.linalg.norm(initpoints.T - numpy.asarray(initpoints[:,k]).reshape(1,2),axis = 1)
                        within1[0][k] = numpy.size(numpy.where(interD < Rori))
                    sorted_agents = numpy.argsort(within1)
                    #print("sorted: " , sorted_agents)
                    #print(sorted_agents)
                    leader_idx = sorted_agents[0][N-N/10:]
                    ...
                    for k in range(numpy.size(leader_idx)):
                        #leader_idx.append(ind_gcm1[0][ind_l1[k]])
                        M.markers[leader_idx[k]].color.r = 1.0                
                
                initpoints,vels =  SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,LeaderPos,leaderRrep,leaderRori,leaderRatt,destination1,destinationthreshold,nol,LeaderVels,destination1,leader_idx)
            ...
            elif(eaten1 == 1 and eaten2 == 0):
            #elif((near_swarm1 == 0 or near_swarm2 == 0 or swarm_split == 0) and (eaten1 == 0 and eaten2 == 0)):#Rori same as Rrep => swarm state
                initpoints,vels =  SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,LeaderPos,leaderRrep,leaderRori,leaderRatt,destination2,destinationthreshold,nol,LeaderVels,destination2,leader_idx)
                ...
                ##print("HERE")
                if(numpy.size(near_dest) > 0):
                    #START EATING 3
                    #print("decreasing") 
                    M.markers[N+nol+2].scale.x = M.markers[N+nol+2].scale.x - size_dec/N*numpy.size(near_dest)
                    M.markers[N+nol+2].scale.y = M.markers[N+nol+2].scale.y - size_dec/N*numpy.size(near_dest)
            ...
            if(abs(LeaderPos[0][1] - LeaderPos[1][1]) <= rs and swarm_split == 0):
                swarm_split = 1;        
            ...
            ##print("g2: " , g2)
                ...
                #START FEEDING
            if(eaten1 == 0):
                reached1 = numpy.size(near_dest1)
                ##print("reached1= " , reached1 , " near_dest1= " , (reached1/numpy.size(g1)) , " total= " , numpy.size(near_dest1))
                M.markers[N+nol].scale.x = M.markers[N+nol].scale.x - size_dec/N*reached1
                M.markers[N+nol].scale.y = M.markers[N+nol].scale.y - size_dec/N*reached1
                if(M.markers[N+nol].scale.x < 0.1):
                    eaten1 = 1; 

            if(eaten2 == 0):
                reached2 = numpy.size(near_dest2)
                M.markers[N+nol+1].scale.x = M.markers[N+nol+1].scale.x - size_dec/N*reached2
                M.markers[N+nol+1].scale.y = M.markers[N+nol+1].scale.y - size_dec/N*reached2
    ...
    print("agents separated: " , agent_sep)
