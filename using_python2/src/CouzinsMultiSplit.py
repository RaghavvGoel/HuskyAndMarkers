...
def main():
    ...
    nol = 2
    leader_pos1 = [-35.0,5.0]
    leader_pos2 = [35.0, 5.0]
    ...
    while not rospy.is_shutdown():
        ...
        elif(near_swarm1 == 0 or near_swarm2 == 0):#Rori same as Rrep => swarm state
            initpoints,vels,notused,notused =  SheepMovement(initpoints,vels,Rrep,Rrep,Ratt,1.5*s1,dt,phi,omega,S,rs,[],leaderRrep,leaderRori,leaderRatt,numpy.asarray([0,0]).reshape(1,2),0,0,[],numpy.asarray([0,0]).reshape(1,2))        

        
        elif(eaten1 == 1 and eaten2 == 1):
                initpoints,vels,LeaderPos,LeaderVels =  SheepMovement(initpoints,vels,Rrep,Rori,Ratt,s1,dt,phi,omega,S,rs,LeaderPos,leaderRrep,leaderRori,leaderRatt,destination,destinationthreshold/3,nol,LeaderVels,destination)                        
        ...
        if(near_swarm1 == 0):
            ...
            #print("sp1= " , sp1)         
            idxD1 = numpy.where(D1 < leaderRori/2.5)            
        ...
        if(near_swarm2 == 0):
            ...
            #print(" sp2= " , sp2)
            idxD2 = numpy.where(D2 < leaderRori/2.5)            
